...
\begin{enumerate}
	...
	We thank the reviewer for their suggestion but it seems quite challenging to make a general statement. As in your comment, if one has coercivity then the convergence is clear. FISTA is known to converge weakly in the Hilbert-space setting, these guarantees are likely to hold here with the added adaptivity although we have not performed the analysis as it is still a relatively special case. A key component of this is the result $\lVert u_n-u_{n-1}\rVert = o(n^{-1})$, in general we do not even expect this quantity to be bounded. It's not clear whether we can leverage the Hilbert-norm structures of FISTA to gain convergence in the Banach space.
	...
	\todo{I could include extra continuous gap (like Figure 1) panels? Or just show continuous gap? At the moment it's perhaps a bit confusing, figures (1,2,3,4,7,9) show (both, discrete, continuous, continuous, discrete, continuous) respectively. It might be simpler just to show the continuous gap after Figure 1 (which compares both).}
\end{enumerate}

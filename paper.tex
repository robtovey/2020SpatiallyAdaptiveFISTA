...
\begin{document}
	...
	}{Clearly $\norm{\nabla\op{f}(\A\varf0^*-\data)}_{\ell^2}$ is a constant, the second term is bounded by
	\begin{equation}
	...
	We consider two phantoms where the ground-truth is either a binary disc or the Shepp-Logan phantom. No noise is added to the Shepp-Logan data but \SI{5}{\percent} \edit{Gaussian white}{Laplace distributed} noise is added to the disc data. This is visualised in Figure~\ref{fig: haar data}. All optimisations shown will be spatially adaptive using Haar wavelets and initialised with $\spcf0^0= \{\varx0\mapsto c\st c\in\F R\}$. The gradient metric shown throughout is the $\ell^2$ norm. Motivated by \eqref{eq: wavelet error metric}, the spatial adaptivity is chosen to refine nodes $j\in\op{leaf}(J_n)$ to ensure that 
	...
	\begin{figure}\centering
		...
		\caption{Phantoms\edit{ and}{,} data \edit{used }{and reconstructions }for wavelet-sparse tomography optimisation. The Shepp-Logan data is exact but the data for the disc-phantom has \SI{5}{\percent}\ \edit{Gaussian white}{Laplace distributed} noise. Without noise the data would be uniform with respect to the angle.}\label{fig: haar data}

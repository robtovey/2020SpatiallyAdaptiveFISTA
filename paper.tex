...
\begin{document}
	...
	\begin{itemize}
		\item we extend the convergence results to the case \edit{$\varf0^*\in\bar{\F H}\setminus\F H$ where $\spcf0\coloneqq \bar{\F H}$ is a Banach space}{the minimum is not achieved. For example, in Lasso discussed in Section~\ref{sec: Lasso definition} there exists a Banach space $\spcf0\supset\F H$ such that $\Emin = \min_{\varf0\in\spcf0}\op{E}(\varf0)$ is achieved in $\spcf0\setminus\F H$}. The proven rate is slower than the optimal $n^{-2}$, but appears to be quite sharp in numerical examples.
	...
	The proposed generalised FISTA algorithm is stated in Algorithm~\ref{alg: refining FISTA} for an arbitrary choice of \edit{refining }{closed }subspaces $\spcf0^n\subset \edit{\spcf0\cap}{}\F H$ for $n=0,1,\ldots$. The only difference \edit{}{from standard FISTA }is that on iteration $n$, all computations are performed in the subspace $\spcf0^n$. If $\spcf0^n\edit{=\spcf0}{}=\F H$ then we recover the \edit{standard FISTA}{classical} algorithm.
	...
	\end{equation}
	is exactly computable for all $\varf1\in\F H$ and all $\tilde{\spcf0}\in\{\spcf0^n\}_{n=0}^\infty$. \edit{}{We denote the orthogonal projection operators $\tens{\Pi}_n\colon\F H\to\spcf0^n$ such that $\tens{\Pi}_n\varf0-\varf0 \perp \varf1$ for all $\varf0\in\F H$, $\varf1\in\spcf0^n$.}
	...
	\end{equation}
	for any norm $\Norm\cdot$.}
	
	\begin{algorithm}\caption{Refining subspace FISTA}\label{alg: refining FISTA}
		\centering
		\begin{algorithmic}[1]
	...
	\begin{adaptiveStep}\label{step: step 1 stability}
		The first step is to generalise \eqref{eq: classical FISTA} to account for the adapting subspaces $\spcf0^n$. In the notation of Algorithm~\ref{alg: refining FISTA}, Theorem~\ref{thm: mini FISTA convergence} shows that 
		...
		\end{equation}
		for $\varf2_n \in \argmin_{\varf0\in\spcf0^n}\op{E}(\varf0)$. The similarities to \eqref{eq: classical FISTA} are clear. If $\spcf0^n=\spcf0$, then we have $\varf2_n=\edit{\varf0^*}{\varf2_0}$ and the two estimates agree. These extra terms quantify the robustness to changing of discretisation.
	\end{adaptiveStep}
	...
	\begin{adaptiveStep}\label{step: step 3 convergence bound}
		In this step we combine the FISTA stability estimate with the subspace approximation guarantees to provide a sharper estimate of stability with respect to the parameters $\vars3_{\op{E}}$ and $\vars3_{\Varf0}$. For example, if $\spcf0^n$ is of the form 
	...
	\edit{}{Throughout this section we consider a sequence of subspaces $\spcf0^n$ which generate the iterates $\varf0_n$ in Algorithm~\ref{alg: refining FISTA} satisfying
		\begin{equation}\label{eq: refining subspace definition}
			\spcf0^n\subset\F H \text{ for } n\in\F N \qquad\text{such that}\qquad \spcf0^n \text{ is closed and }\varf0_n\in\spcf0^{n+1}\text{ for all }n\in\F N
		\end{equation}
		with orthogonal projections $\tens{\Pi}_n\colon\F H\to\spcf0^n$.
	}
	...
	\begin{lemma}[equivalent to {\cite[Lemma \edit{1}{3.1}]{Chambolle2015}}]\label{thm: descent lemma}
		\Copy{thm: descent lemma}{
			...
			$$\op{E}(\varf0) + \tfrac12\norm{\varf0-\varf2}^2 \leq \op{E}(\varf2) + \tfrac12\norm{\tens{\Pi}_n\bar{\varf0}-\varf2}^2 \leq \op{E}(\varf2) + \tfrac12\norm{\bar{\varf0}-\varf2}^2.$$}
			\edit{where $\tens{\Pi}_n\colon\bar{(\spcf0^n)^*}\to\spcf0^n$ is the orthogonal projection.}{}
		}
	\end{lemma}
	\noindent This is exactly the result of \cite{Chambolle2015} applied to the function $\varf0\mapsto\op{E}(\tens{\Pi}_n\varf0)$. Applying Lemma~\ref{thm: descent lemma} to the iterates from Algorithm~\ref{alg: refining FISTA} gives a more explicit inequality.
	
	\begin{lemma}[{\edit{analogous to }{}\cite[\edit{Theorem 2}{(17)}]{Chambolle2015}, \cite[\edit{Theorem 1}{Lmma 4.1}]{Beck2009}}]\label{thm: one step FISTA}
	...
	\begin{theorem}[\edit{}{analogous to {\cite[Thm 3.2]{Chambolle2015}, \cite[Thm 4.1]{Beck2009}}}]\label{thm: mini FISTA convergence}
		\Copy{thm: mini FISTA convergence}{
			Fix a sequence of subspaces $(\spcf0^n\edit{\subset\spcf0\cap\F H}{})_{n\in\F N}$ \edit{}{satisfying \eqref{eq: refining subspace definition}}, arbitrary $\varf0_0\in \spcf0^0$, and FISTA stepsize choice $(t_n)_{n\in\F N}$. Let $\varf0_n$ and $\varf1_n$ be generated by Algorithm~\ref{alg: refining FISTA}. 
	...
	\end{theorem}
	\noindent The proof is given in Theorem~\ref{app:thm: mini FISTA convergence}. This result is the key approximation for showing convergence of FISTA with refining subspaces. In the classical setting, we have $\spcf0^n\edit{=\spcf0}{}=\F H$, $\varf2_n\edit{=\varf0^*}{\in\argmin_{\varf0\in\F H}\op{E}(\varf0)}$ and the extra terms on the right-hand side collapse to 0. 
	
	\edit{The}{If there exists a minimiser $\varf0^*\in\argmin_{\varf0\in\F H}\op{E}(\varf0)$, then the } natural choice in \eqref{eq: FISTA inequality} is $\varf2_n=\tens{\Pi}_n\varf0^*$, however, there are simple counter-examples which give $\op{E}(\tens{\Pi}_n\varf0^*)=\infty$ and so this inequality becomes useless. For example, if $\op{f}(\varf0) = \norm{\varf0}_{\edit{2}{L^2([0,1])}}^2$, $\op{g}$ is the indicator on the set $\F D=\{\varf0\in L^1([0,1])\st \varf0(\varx0) \geq \varx0\}$, and $\spcf0^n$ is a set of piecewise constant functions, then $\varf0^*=\varx0\mapsto \varx0$. On the other hand, suppose one of the pixels of the discretisation is $[\varx0_0-\meshsize,\varx0_0+\meshsize]$, then 
	$$\tens{\Pi}_n\varf0^*\left(\varx0_0+\sfrac\meshsize2\right) = \argmin_{\vars5\in\R}\int_{\varx0_0-\meshsize}^{\varx0_0+\meshsize}(\varf0^*(\varx0)-\vars5)^2d\varx0 = \argmin_{\vars5\in\R}\int_{\varx0_0-\meshsize}^{\varx0_0+\meshsize}(\varx0-\vars5)^2d\varx0 = \varx0_0 < \varx0_0+\sfrac\meshsize2.$$
	In particular $\tens{\Pi}_n\varf0^* \notin\F D$ therefore $\op{E}(\tens{\Pi}_n\varf0^*) = \infty$. The choice $\varf2_n=\argmin_{\varf0\in\spcf0^n}\op{E}(\varf0)$ is much more robust and allows us to apply Algorithm~\ref{alg: refining FISTA} more broadly. The penalty for this flexibility is a more complicated analysis, each time the subspace changes, the system receives a `shock' proportional to $\norm{\varf2_{n}-\varf2_{n+1}}$. 
	...
	\begin{definition}\label{def: exp subspaces}
		Fix $\vars3_{\Varf0},\vars3_{\op{E}}\geq 1$ and a sequence of \edit{subspaces $(\tilde{\spcf0}^k\subset\F H\cap\spcf0)_{k\in\F N}$}{closed subspaces $\tilde{\spcf0}^k\subset\F H$}. We say that $(\tilde{\spcf0}^k)_{k\in\F N}$ is a \emph{$(\vars3_{\Varf0},\vars3_{\op{E}})$-discretisation for $\op{E}$} if 
	...
	\begin{lemma}\label{thm: practical refinement criteria}
		\Copy{thm: practical refinement criteria}{
			Let $(\tilde{\spcf0}^k)_{k\in\F N}$ be a sequence of subspaces with some points $\varf0_k\in \tilde{\spcf0}^k$ and $\tilde{\varf2}_k\in\argmin_{\varf0\in\tilde{\spcf0}^k}\op{E}(\varf0)$. Suppose that $\norm{\tilde{\varf2}_k}\lesssim \vars3_{\Varf0}^k$. Any of the following conditions are sufficient to show that $\{\tilde{\spcf0}^k\}$ is an $(\vars3_{\Varf0},\vars3_{\op{E}})$-discretisation for $\op{E}$:
			\begin{enumerate}
			...
			\end{enumerate}
			\edit{}{If, furthermore, there exists a Banach space $(\spcf0,\Norm\cdot)$ with $\spcf0\supset\F H$, then it is also sufficient that:}
			\begin{enumerate}\setcounter{enumi}{3}
				\item Small continuous gradient refinement: $ \Norm{\partial\op{E}(\varf0_k)}_*\leq \vars1\vars3_{\op{E}}^{-k}$ for all $k\in\F N$, some $\vars1>0$, and sublevel sets of $\op{E}$ are $\Norm\cdot$-bounded.
				\item Small discrete gradient refinement: $\op{E}_0(\tilde{\varf2}_k)\leq \vars1\vars3_{\op{E}}^{-k}$ and $\Norm{\tens{\Pi}_k\partial\op{E}(\varf0_k)}_*\leq \vars1\vars3_{\op{E}}^{-k}$ for all $k\in\F N$, some $\vars1>0$, and sublevel sets of $\op{E}$ are $\Norm\cdot$-bounded.
			\end{enumerate}
		}
	\end{lemma}
	...
	for some $\varf0^*\in\spcf0$. The cases where $\F H$ has finite or countable dimension are quite straightforward. If the total number of refinements is finite (i.e. $\spcf0^n = \spcf0^N$ for all $n\geq N$, some $N\in\F N$) then $\vars3_{\Varf0} = 1$. This holds for most finite dimensional problems as well as the countable Lasso example discussed in detail in Section~\ref{sec: Lasso definition}. In this section we give explicit computations of $\vars3_{\Varf0}$ and $\vars3_{\op{E}}$ in the setting where $\F H = L^2(\Domain)$ for some domain $\Domain\subset\R^d$ and the subspaces $(\tilde{\spcf0}^n)_{n\in\F N}$ will be finite dimensional finite-element--like spaces, as defined below.
	}
			
	\begin{definition}
		Suppose $\spcf0\subset L^p(\Domain)$ for some \edit{compact}{connected, bounded} domain $\Domain\subset\R^d$\edit{}{ with sufficiently smooth boundary}. We say that $\F M^k = \{\domain_1^k,\domain_2^k,\ldots\}$ is a \emph{mesh} if 
	...
	Application of Theorem~\ref{thm: generic a_U and a_E} gives $\vars3_{\Varf0}=2^{\frac d2}$ although \edit{although}{} $\norm\cdot_1$ is too strong a metric and results in $\vars3_{\op{E}}=2^0$. This can be seen because for any $\varf0\in L^1(\Domain)$ and Dirac mass $\delta$ supported in $(0,1)^d$, 
	\begin{equation}
		\Norm{\varf0-\delta} = \sup_{\vard0\in C(\Domain), \norm{\vard0}_{L^\infty}\leq 1} \IP{\vard0}{\varf0-\delta} = \Norm{\varf0}+\Norm{\delta} \geq 1.
	\end{equation}
	To improve our estimate of $\vars3_{\op{E}}$ requires additional assumptions on $\A$. For any $\varf2\in\tilde{\spcf0}^k$ such that $\Norm{\varf2}\leq \Norm{\varf0^*}$, we compute
	\begin{align}
	...
	}{Piecewise constant projection is $L^1$-bounded therefore we can extend $\tilde{\tens{\Pi}}_k\colon\spcf0\to\tilde{\spcf0}^k$ by the Hahn-Banach theorem. Choosing $\varf2=\tilde{\tens{\Pi}}_k\varf0^*$ gives
	\begin{align}
		\op{E}(\varf2)-\op{E}(\varf0^*) &\lesssim \norm{\A(\tilde{\tens{\Pi}}_k\varf0^*-\varf0^*)}_{\ell^2}
	...
	$$\tilde{\spcf0}^k = \{\varf0\in \ell^1(\R) \st i\notin J_k\implies \varf0_i=0\}$$ for some sets $J_k\subset\F N$, i.e. infinite vectors with finitely many non-zeros. The key change in analysis from the continuous case is $\varf0^*\in\ell^2(\R)$, which leads to $\vars3_{\Varf0}=1$ and the expected optimal rate of $n^{-2}$, independent of $\vars3_{\op{E}}$ or any additional properties of $\A$. The number of refinements will also be finite, therefore $n_k=\infty$ for some $k$ and the remaining conditions of Theorems~\ref{thm: exponential FISTA convergence} and~\ref{thm: stronger exponential FISTA convergence} hold trivially.
	...
	$$ \text{if}\quad \op{E}(\varf0_n) + \op{E}^*(\vars2_0 \vvard0_n) \approx 0 \quad \text{then}\quad \left\{\vvarx0\st |[\A^*\vvard0_n](\vvarx0)|\ll \mu\right\} \subset\{\vvarx0 \st \varf0^*(\vvarx0)=0\}.$$
	...
	Therefore,
	\begin{equation}
		|[\A^*\vars2_0\vvard0_n](\vvarx0)|< \mu - \sqrt{2(\op{E}(\varf0_n)+\op{E}^*(\vars2_0\vvard0_n))}|\A^*|_{\ell^2\to L^\infty} \qquad \implies \qquad \varf0^*(\vvarx0)= 0.\label{eq: support equation}
	\end{equation}
	...
	\begin{itemize}
		\item The convergence guarantee from Theorem~\ref{thm: exponential FISTA convergence} is for the quantity $\op{E}(\varf0_n)-\op{E}(\varf0^*)$, the more relevant quantity here is $\op{E}^*(\vars2_0\vvard0_n)+\op{E}(\varf0^*)$ for which there is no proven rate.
		\item In Section~\ref{sec: continuous lasso rate}, $|\A^*|_{\ell^2\to C^1}<\infty$ was required to compute a rate of convergence, but only $|\A^*|_{\ell^2\to L^\infty}<\infty$ is needed to estimate the support.
	\end{itemize}
	...
	\begin{proof}
		...
		\end{equation}
		therefore it is sufficient to bound $\frac{\Norm\cdot_*}{\norm\cdot}$ on the subspace $\tilde{\spcf0}$. Switching to the given basis, for $\varf0 = \sum_{j=1}^{\op{dim}(\tilde{\spcf0})} \varx3_je_j$ we have
		\begin{align}
			\norm{\varf0}^2 &= \sum_{j=1}^{\op{dim}(\tilde{\spcf0})} \varx3_j^2 = \norm{\vvarx3}_{\ell^2}^2,
			\\\Norm{\varf0}_*&\leq \sum_{j=1}^{\op{dim}(\tilde{\spcf0})} |\varx3_j|\Norm{e_j}_* \leq \max_{j\leq\op{dim}(\tilde{\spcf0})} \Norm{e_j}_*\norm{\vvarx3}_{\ell^1},
			\\\implies \frac{\Norm{\varf0}_*}{\norm{\varf0}} &\leq \max_{j\leq \op{dim}(\tilde{\spcf0})}\Norm{e_j}_*\frac{\norm{\vvarx3}_{\ell^1}}{\norm{\vvarx3}_{\ell^2}} \leq \max_{j\leq \op{dim}(\tilde{\spcf0})}\Norm{e_j}_*\sqrt{\op{dim}(\tilde{\spcf0})}.
		\end{align}

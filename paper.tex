...
\begin{document}
	...
	\begin{itemize}
		\item we extend the convergence results to the case \edit{$\varf0^*\in\bar{\F H}\setminus\F H$ where $\spcf0\coloneqq \bar{\F H}$ is a Banach space}{where the minimum is not achieved. For example, in \edit{Lasso}{L1 penalised reconstructions} discussed in Section~\ref{sec: Lasso definition} there exists a Banach space $\spcf0\supset\F H$ such that $\Emin = \min_{\varf0\in\spcf0}\op{E}(\varf0)$ is achieved in $\spcf0\setminus\F H$}. The proven rate is slower than the optimal $n^{-2}$, but appears to be quite sharp in numerical examples.
	...
	The proposed generalised FISTA algorithm is stated in Algorithm~\ref{alg: refining FISTA} for an arbitrary choice of \edit{refining subspaces}{closed convex subsets} $\spcf0^n\subset \edit{\spcf0\cap}{}\F H$ for $n\in\F N$. The only difference \edit{}{from standard FISTA }is that on iteration $n$, all computations are performed in the \edit{subspace}{subset} $\spcf0^n$. If $\spcf0^n\edit{=\spcf0}{}=\F H$, then we recover the \edit{standard FISTA}{original} algorithm. \edit{}{More generally, the idea is that $\spcf0^n$ are `growing', for example $\spcf0^n\subset\spcf0^{n+1}$, but this assumption is not necessary in most of the results.}
	...
	\end{equation}
	is exactly computable for all $\varf1\in\F H$ and all $\tilde{\spcf0}\in\{\spcf0^n\}_{n=0}^\infty$. \edit{}{Closed subsets of $\F H$ are locally weakly compact, therefore this argmin is always non-empty.}
	...
	\begin{adaptiveStep}\label{step: step 2 smoothness}
		To show that the extra terms in \eqref{eq: mini FISTA convergence} are small, we need to quantify the approximation properties of $\spcf0^n$. \edit{}{The idea is that $\spcf0^n$ are locally compact, so that minimisers are achieved, but `growing' so that $\min_{\varf0\in\spcf0^n}\op{E}(\varf0) \to \inf_{\varf0\in\F H}\op{E}(\varf0)$. A canonical example might be $\spcf0^n=\{\varf0\in\F H\st \norm{\varf0}\leq n\}$.} \edit{To do so, we construct}{To quantify the balance between a growing Hilbert norm and improving estimate of $\op{E}$, we introduce} a secondary sequence $n_0 < n_1 < \ldots$ and constants $\vars3_{\Varf0},\vars3_{\op{E}}\geq1$ such that for each $k\in\F N$
		\begin{equation}
			n\leq n_k \implies \norm{\varf2_n} \lesssim \vars3_{\Varf0}^k, \qquad n\geq n_k \implies \op{E}_0(\varf2_n) \lesssim \vars3_{\op{E}}^{-k}.
		\end{equation}
		\edit{}{The expanding balls example would become $\spcf0^n=\{\varf0\in\F H\st \norm{\varf0}\leq \vars3_{\Varf0}^k\}$, and $\vars3_{\op{E}}$ reflects the smoothness of $\op{E}$ with respect to the Hilbert norm.} The choice of exponential scaling is \edit{very}{more} natural if we consider the $\spcf0^n$ to be the subspace of functions discretised on a uniform mesh. If that mesh is sequentially refined, then the resolution of the mesh will be of order $h^k$ after $k$ refinements and for some $h<1$. The integer $n_k$ should then be the time when the mesh has refined $k$ times. %
		\edit{The value of $\vars3_{\Varf0}$ is dictated by the smoothness of the norm of $\spcf0$ with respect to that of $\F H$. If $\norm{\varf0^*}<\infty$, then the value is $\vars3_{\Varf0}=1$, otherwise there is some damage to the rate of convergence generated by this factor. On the other hand, the value of $\vars3_{\op{E}}$ is dictated by the smoothness of $\op{E}$ at the point $\varf0^*$.}{The value $\vars3_{\Varf0}$ reflects the smoothness of the chosen basis measured in $\F H$, and $\vars3_{\op{E}}$ is a measure of efficiency with respect to the structure of $\op{E}$.} %
		If an efficient basis is used, then $\vars3_{\op{E}}$ is large. The trade-off between $\vars3_{\op{E}}$ and $\vars3_{\Varf0}$ dictates the final convergence rate of the algorithm. \edit{}{If $\vars3_{\Varf0}>1$, then we cannot guarantee the original $n^{-2}$ rate of convergence.}
	\end{adaptiveStep}
	...
	\edit{}{Throughout this section we consider a sequence $(\spcf0^n)_{n\in\F N}$ which generate the iterates $(\varf0_n)_{n\in\F N}$ in Algorithm~\ref{alg: refining FISTA} such that
	\begin{equation}\label{eq: refining subspace definition}
		\varf0^{n-1}\in\spcf0^n\subset\F H \quad\text{where }\spcf0^n\text{ is a closed, convex subset for all } n\in\F N.
	\end{equation}}
	...
	\end{lemma}
	\noindent The proof is exactly the same as in \cite{Chambolle2015} on the \edit{subspace}{subset} $\spcf0^n$. Applying Lemma~\ref{thm: descent lemma} to the iterates from Algorithm~\ref{alg: refining FISTA} gives a more explicit inequality.
	...
	\begin{align}
		\op{E}(\varf2)-\op{E}(\varf0) &= \edit{\tfrac12\norm{\A\varf2-\data}_{\ell^2}^2 - \tfrac12\norm{\A\varf0-\data}_{\ell^2}^2}{\op{f}(\A\varf2-\data)-\op{f}(\A\varf0-\data)} + \mu\left(\Norm{\varf2}-\Norm{\varf0}\right)
		\\&\leq \ip{\nabla \op{f}(\A\varf2-\data)}{\A(\varf2-\varf0)}
		\\&\leq\edit{\max(\norm{\A\varf2-\data}_{\ell^2},\norm{\A\varf0-\data}_{\ell^2})}{\left[\norm{\nabla\op{f}(\A\varf0^*-\data)}_{\ell^2} + \norm{\A(\varf2-\varf0^*)}_{\ell^2}\right]}\norm{\A(\varf2-\varf0)}_{\ell^2}.
	\end{align}
	...
	}{Clearly $\norm{\nabla\op{f}(\A\varf0^*-\data)}_{\ell^2}$ is a constant. For the second term, recall $\Norm{\varf2}\leq \Norm{\varf0}$, therefore we can bound
	\begin{equation}
	\norm{\A(\varf2-\varf0^*)}_{\ell^2} = \sup_{\vvarx3\in\R^m}\norm{\vvarx3}_{\ell^2}^{-1} \IP{\A^*\vvarx3}{\varf2-\varf0^*} \leq \sup_{\vvarx3\in\R^m}\norm{\vvarx3}_{\ell^2}^{-1} |\A^*\vvarx3|_{L^\infty}\Norm{\varf2-\varf0^*} \leq |\A^*|_{\ell^2\to C^0}(\Norm{\varf0}+\Norm{\varf0^*}).
	\end{equation}
	...
	where $\sqrt d2^{-k}$ is the diameter of a $d$ dimensional pixel of side length $2^{-k}$. \edit{}{Sub-levelsets of $\op{E}$ are compact in $L^1(\Domain)$, therefore there exists a weak$^*$-converging minimising sequence $\varf0^*_j\in\F H$ such that $\varf0^*_j\rightharpoonup\varf0^*$ and $\lim_{j\to\infty}\op{E}(\varf0^*_j)=\op{E}(\varf0^*)$. 
		...
		\begin{equation}
			\inf_{\varf2\in\tilde{\spcf0}^k} \op{E}(\varf2)-\op{E}(\varf0^*) = \lim_{j\to\infty} \inf_{\varf2\in\tilde{\spcf0}^k} \op{E}(\varf2)-\op{E}(\varf0_j^*) \leq \lim_{j\to\infty} \op{E}(\tilde{\tens{\Pi}}_k\varf0_j^*)-\op{E}(\varf0_j^*) \lesssim 2^{-k}.
		\end{equation}}
	...
	\begin{align}
		\min_{\varf0\in \spcf0^n} \edit{\tfrac12\norm{\A\varf0-\data}_{\ell^2}^2}{\op{f}(\A\varf0-\data)} + \mu\Norm{\varf0} &= \min_{\varf0\in\F H}\max_{\vvard0\in\R^m} \ip{(\A\tens{\Pi}_n\varf0-\data)}{ \vvard0} +\mu\Norm{\tens{\Pi}_n\varf0} -\edit{\tfrac12\norm{\vvard0}_{\ell^2}^2}{\op{f}^*(\vvard0)}
		\\&= \max_{\vvard0\in\R^m}\min_{\varf0\in\F H} \ip{(\A\tens{\Pi}_n\varf0-\data)}{\vvard0} +\mu\Norm{\tens{\Pi}_n\varf0} -\edit{\tfrac12\norm{\vvard0}_{\ell^2}^2}{\op{f}^*(\vvard0)}
		\\&= \max_{\vvard0\in\R^m} \splitln{-\ip{\data}{\vvard0} -\edit{\tfrac12\norm{\vvard0}_{\ell^2}^2}{\op{f}^*(\vvard0)}}{\qquad\Norm{\tens{\Pi}_n\A^*\vvard0}_*\leq \mu}{-\infty}{\qquad\text{else}}
		\\&= -\min_{\vvard0\in\R^m} \underbrace{\edit{\tfrac12\norm{\vvard0}_{\ell^2}^2}{\op{f}^*(\vvard0)}+\ip{\data}{\vvard0}}_{\eqqcolon \op{E}^*(\vvard0)}  + \chi(\Norm{\tens{\Pi}_n\A^*\vvard0}_*\leq \mu).
	\end{align}
	In particular,
	\begin{equation}
	...
	\begin{equation}
		\edit{}{\A\varf0^*-\data\in\partial\op{f}^*(\vvard0^*),\qquad\text{or equivalently}\qquad}
		\vvard0^* = \edit{}{\nabla\op{f}}(\A\varf0^*-\data).
	\end{equation}
	...
	\begin{equation}
		\vvard0_n\coloneqq \edit{}{\nabla\op{f}}(\A\varf0_n - \data), \qquad \op{E}(\varf0)-\min_{\varf0'\in\spcf0^n}\op{E}(\varf0') \leq \op{E}(\varf0) +\edit{\min_{\vars2\geq 0}\{}{}\op{E}^*(\vars2\vvard0_n) \edit{\st \vars2 \Norm{\tens{\Pi}_n\A^*\vvard0_n}_*\leq \mu\}}{}
	\end{equation}
	\edit{with optimal choice}{for some $0\leq \vars2\leq\frac{\mu}{\Norm{\tens{\Pi}_n\A^*\vvard0_n}_*}$. In the case $\op{f}(\cdot)=\frac12\norm{\cdot}_{\ell^2}^2$, one can use the optimal choice}
	\begin{equation}
		\vars2 =\max\left(0, \min\left(\frac{-\ip{\data}{\vvard0_n}}{\norm{\vvard0_n}_{\ell^2}^2}, \frac{\mu}{\Norm{\tens{\Pi}_n\A^*\vvard0_n}_*}\right)\right).
	\end{equation}
	To apply Algorithm~\ref{alg: refining FISTA}, we are assuming that both $\op{f}(\A\varf0_n-\data)\edit{ = \frac12\norm{\vvard0_n}_{\ell^2}^2}{}$ and $\tens{\Pi}_n\nabla\op{f}(\A\varf0_n-\data)\edit{ = \tens{\Pi}_n\A^*\vvard0_n}{}$ are easily computable, therefore $\vars2$ and $\op{E}(\varf0_n) +\op{E}^*(\vars2\vvard0_n)$ are also easy to compute.
	...
	\begin{align}
		...
		\end{cases}
		\\ \op{E}_0(\varf0_n) &\leq \op{E}(\varf0_n) + \op{E}(\vars2_0\vvard0_n), \qquad \vars2_0\edit{=\max\left(0, \min\left(\frac{-\ip{\data}{\vvard0_n}}{\norm{\vvard0_n}_{\ell^2}^2}, \frac{\mu}{\Norm{\A^*\vvard0_n}_*}\right)\right)}{\in\left[0,\frac{\mu}{\Norm{\A^*\vvard0_n}_*}\right]}
	\end{align}
	where $\vvard0_n = \edit{}{\nabla\op{f}}(\A\varf0_n-\data)\in\R^m$ is always exactly computable.
	...
	\edit{First we use the strong convexity of $\op{E}^*$}{Recall that $\nabla\op{f}$ is 1-Lipschitz if and only if $\op{f}^*$ is 1-strongly convex \cite[Chapter 10, Thm. 4.2.2]{Hiriart2013}. Therefore}, if $\vars2_0\vvard0_n$ and $\vvard0^*$ are both dual-feasible, then
	\begin{equation}
	...
	\begin{equation}
		\F{X}_i^I = \left\{\vvarx0\in[-\tfrac12,\tfrac12]^2\st \ip{\vvarx0}{\begin{pmatrix}\cos\theta_I\\\sin\theta_I\end{pmatrix}}\in \left[-\tfrac12+\tfrac{i-1}{100},-\tfrac12+\tfrac{i}{100}\right)\right\}, \quad \theta_I=\frac{180^\circ}{51}I
	\end{equation}
	...
	\begin{equation}
		\norm{\A}_{L^2\to\ell^2} \leq \sqrt{\sum_{I\in[50]}\max_{i\in[100]} |\F{X}^I_i|} = \sqrt{\sum_{I\in[50]}\max_{i\in[100]} \int_{\F{X}^I_i}1d\vvarx0}= \sqrt{\sum_{I\in[50]}\max_{i\in[100]}\ (\A\1)_{i,I}}\;.
	\end{equation}
	...
	\begin{equation}
		\min_{\varf0\in \spcf0} \edit{\tfrac12\norm{\A\varf0-\data}_{\ell^2}^2}{\op{f}(\A\varf0-\data)} + \mu \norm{\tens{W}^{-1}\varf0}_{\ell^1} = \min_{\hat{\varf0}\in\ell^1(\R)} \edit{\tfrac12\norm{\A\tens{W}\hat{\varf0}-\data}_{\ell^2}^2}{\op{f}(\A\tens{W}\hat{\varf0}-\data)} + \mu \norm{\hat{\varf0}}_{\ell^1}.
	\end{equation}
	...
	\edit{}{ In this example we consider the smoothed robust fidelity \cite{Rosset2007}
		\begin{equation}
			\op{f}(\vvard0) = \sum_{i=1}^m \splitln{10^{-4}|\vard0_i|}{|\vard0_i|\geq10^{-4}}{\tfrac12|\vard0_i|^2+\tfrac12 10^{-8}}{\text{else}} \approx 10^{-4}\norm{\vvard0}_{\ell^1}.
		\end{equation}
	}	
	...
	We consider two phantoms where the ground-truth is either a binary disc or the Shepp-Logan phantom. \edit{No noise is added to the Shepp-Logan data but 5\% Gaussian white noise is added to the disc data.}{Both examples are corrupted with \SI{2}{\percent} Laplace distributed noise.} This is visualised in Figure~\ref{fig: haar data}. All optimisations shown are spatially adaptive using Haar wavelets and initialised with $\spcf0^0= \{\varx0\mapsto c\st c\in\F R\}$. The gradient metric shown throughout is the $\ell^\infty$ norm. Motivated by \eqref{eq: wavelet error metric}, the spatial adaptivity is chosen to refine nodes $j\in\op{leaf}(J_n)$ to ensure that 

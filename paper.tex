...
\begin{adaptiveStep}\label{step: step 5 sufficiently slow}
	...
	$$ \Func0_0(\varf0_N) \lesssim \frac{\vars3_{\Varf0}^{2K}}{N^2} \lesssim \frac{N^{2\epsilon}}{N^2}$$
	for all $N\in\F N$, some $\epsilon\in[0,1)$ depending on $\vars3_{\Varf0},\vars3_{\Func0}$. In particular, if $\varf0^*\in\F H$ then we recover the classical rate with $\epsilon=0$.
\end{adaptiveStep}
...
\begin{adaptiveStep}\label{step: step 6 adaptivity}
	...
	for all $N\in\F N$, with the same $\epsilon\in[0,1)$ from \Cref{step: step 5 sufficiently slow}. The penalty for accelerating the refinement time is a potential loss of stability in $\Func0(\varf0_n)$, however, the asymptotic rate is equivalent and this behaviour has not been seen in numerical experiments.
\end{adaptiveStep}
...
For \Cref{step: step 1 stability} of \Cref{sec: recipe} we look to replicate the classical bound of the form in \eqref{eq: classical FISTA} for \Cref{alg: refining FISTA}. The proofs in this step follow the classical arguments of \citep{Beck2009,Chambolle2015} very closely. 
...
\end{lemma}
\noindent This is exactly the result of \citep{Chambolle2015} applied to the function $\varf0\mapsto\Func0(\Pi_n\varf0)$. Applying \Cref{thm: descent lemma} to the iterates from \Cref{alg: refining FISTA} gives a more explicit inequality.
...
\begin{lemma}\label{thm: mini exponential FISTA convergence}
	\Copy{thm: mini exponential FISTA convergence}{
		Let $\varf0_n$, $\varf1_n$ be generated by \Cref{alg: refining FISTA}, $(n_k\in\F N)_{k=0}^\infty$ be a monotone increasing sequence, and define
...
\begin{lemma}\label{thm: sufficiently fast}
	\Copy{thm: sufficiently fast}{
		Suppose $\spcf0^n, \varf0_n, \varf1_n$ and $n_k$ satisfy the conditions of \Cref{thm: mini exponential FISTA convergence} and $\{\tilde{\spcf0}^k\}$ forms an $(\vars3_{\Varf0},\vars3_{\Func0})$-discretisation for $\Func0$.
		If either:
		\begin{itemize}
...
As a special case, note that if $\vars3_{\Varf0}=1$ then \Cref{thm: sufficiently fast} already gives the optimal $O(N^{-2})$ convergence rate. This is in fact a special case of \citep{Aujol2015}. If $\varf0^*\in\F H$ then it is not possible to refine `too quickly' and the following lemma is not needed.
...
\Cref{thm: sufficiently fast} gives a sufficient condition on $n_k$ for converging at the rate $O(N^{2(\epsilon-1)})$ but it is not necessary. Indeed for $n\leq n_k$ the result shows 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage {\Huge\todo{I don't know if this section should remain}}
...
\begin{proof}
	Inserting the assumed rates into \Cref{app:thm: mini FISTA convergence} gives
	\begin{equation}

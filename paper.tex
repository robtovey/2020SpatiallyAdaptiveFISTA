...
\edit[0]{%
	...
	There is much overlap between the techniques used in this work and those used in the literature of inexact optimisation, however, our interpretation is relatively novel. In particular, we emphasise the infinite-dimensional setting where errors come from `discretisation' (rather than random or decaying errors in $\F H$), which enables two new perspectives:
	\begin{itemize}
		\item Analytically, we prove new rates of convergence for FISTA when the minimum energy is not achieved (at least not in $\F H$). The exact rate can be computed by quantifying coercivity and regularity properties of $\op{E}$. If there isn't a minimiser in $\F H$, then this rate is strictly smaller than $n^{-2}$.
	...
	\begin{adaptiveStep}\label{step: step 3 convergence bound}
		...
		$$\edit{\F{U}^{n_k} = \F{U}^{n_k+1} = \ldots = \F{U}^{n_{k+1}-1}}{\var2_{n_k}=\var2_{n_k+1}=\ldots \var2_{n_{k+1}-1}},$$
		then many terms on the right-hand side of \eqref{eq: mini FISTA convergence} telescope to 0. The result of this is presented in Lemma~\ref{thm: mini exponential FISTA convergence}. The key idea is that the stability error in \eqref{eq: mini FISTA convergence} has $K\lesssim\log(N)$ terms, rather than $N$.
	\end{adaptiveStep}
	...
	\begin{adaptiveStep}\label{step: step 6 adaptivity}
		...
		The main challenge for adaptive choice of $\F{U}^n$ is to guarantee \eqref{eq: first regularity} from Step~\ref{step: step 2 smoothness} using a posteriori estimates. Combined with the partial telescoping requirement in Step~\ref{step: step 3 convergence bound}, a natural choice is $\var2_n=\var0_{n_k-1}$ for $n\in[n_k,n_{k+1}-1)$, i.e. the value of $n_k$ is chosen to be $n+1$ once the iterate $\var0_n$ is observed.
		}
	...
	\end{theorem}
	\noindent The proof is given in Theorem~\ref{app:thm: mini FISTA convergence}. This result is the key approximation for showing convergence of FISTA with growing subsets. In the classical setting, we have $\F{U}^n=\F H$, $\var2_n=\var2_0\in\argmin_{\var0\in\F H}\op{E}(\var0)$ and the extra terms on the right-hand side collapse to 0. 
	...
	\begin{itemize}
		...
		\item If $\F H$ is finite dimensional, then the $\aU=1$ is almost trivially satisfied. Norms in finite dimensions are equivalent and any discretisation can be achieved with a finite number of refinements (i.e. the sums over $k$ are finite). 
	\end{itemize}
	...
	\begin{lemma}\label{thm: sufficiently slow}
		\Copy{thm: sufficiently slow}{
			...
			$$\op{E}_0(\var0_N) \lesssim \frac{\aU^{2K}}{N^2} \qquad\text{where} \qquad  n_K^2\gtrsim \aE^K\aU^{2K},$$
			then 
	...
	\end{theorem}
	\noindent \edit{}{Analytically, this theorem gives new rates of convergence for FISTA when the minimiser is not achieved in the Hilbert space. Indeed for the original algorithm ($\F{U}^n=\F H$), if $\var0=0$ for simplicity and $(\tilde{\var2}_k)_{k\in\F N}$ is \emph{any} \aUaE\ exists, the result of Lemma~\ref{thm: mini exponential FISTA convergence} is
	...
	\end{equation}
	In this sense, we could say that $\op{E}$ converges at the rate $N^{-2(1-\kappa)}$ if and only if such a sequence exists. Nothing is lost (or gained) by choosing $\F{U}_n\subsetneq\F H$.}
	...
	\begin{theorem}\label{thm: stronger exponential FISTA convergence}
		\Copy{thm: stronger exponential FISTA convergence}{
			Let $(\F{U}^n\subset\F H)_{n\in\F N}$ be a sequence of subsets satisfying \eqref{eq: refining subspace definition}\edit{}{, $(\tilde{\var2}_k)_{k\in\F N}$ an \aUaE,} and $n_k\in\F N$ a monotone increasing sequence such that
			$$ \edit{\tilde{\F{U}}^k \coloneqq \F{U}^{n_k}, \qquad}{} \tilde{\var2}_k \in \edit{\left[\argmin_{\var0\in\tilde{\F{U}}^k}\op{E}(\var0)\right]}{\F{U}^{n_k}}\cap\F{U}^{n_k+1} \cap\ldots\cap \F{U}^{n_{k+1}-1}$$
			for all $k\in\F N$.
			...
			\sout{Whenever these conditions on $n_k$ are satisfied, }}{If $\aU>1$, $\aE\geq1$, and $n_k^2\lesssim \aE^k\aU^{2k}$, }then
	...
	To enable a more practical implementation of Theorem~\ref{thm: stronger exponential FISTA convergence}, the following lemma describes several refinement time strategies which provide sufficient condition for $\op{E}_0(\edit{\var0_n}{\tilde{\var2}_k}) \lesssim \aE^{-k}$.
	\begin{lemma}\label{thm: practical refinement criteria}
		\Copy{thm: practical refinement criteria}{
			...
			\begin{enumerate}
				...
				\item Small relative gap refinement: $ \op{E}_0(\edit{\var0_k}{\tilde{\var2}_k}) - \op{E}_0(\edit{\tilde{\var2}_{k-1}}{\tilde{\F{U}}^{k-1}}) \leq \vars1\op{E}_0(\edit{\var0_k}{\tilde{\var2}_k})$ for all $k\in\F N$, some $0<\vars1\leq \frac{1}{1+\aE}$.
			\end{enumerate}
			\edit{If}{Suppose}, furthermore, there exists a Banach space $(\F{U},\Norm\cdot)$ with $\F{U}\supset\F H$ and \edit{}{dual-norm $\Norm\cdot_*$. If} the sublevel sets of $\op{E}$ are $\Norm\cdot$-bounded, then it is also sufficient that:
			\begin{enumerate}\setcounter{enumi}{3}
				\item Small continuous gradient refinement: $ \inf_{\var1\in\partial\op{E}(\edit{\var0_k}{\tilde{\var2}_k})}\Norm{\var1}_*\leq \vars1\aE^{-k}$ for all $k\in\F N$, some $\vars1>0$.
				\item Small discrete gradient refinement: $\op{E}_0(\edit{\tilde{\var2}_k}{\tilde{\F{U}}^k})\leq \vars1\aE^{-k}$ and $\sup_{\var0,\tilde{\var0}\in \tilde{\F{U}}^k}\inf_{\var1\in\partial\op{E}(\var0_k)}\frac{|\IP{v}{\var0-\tilde{\var0}}|}{\Norm{\var0-\tilde{\var0}}}\leq \vars1\aE^{-k}$ for all $k\in\F N$, some $\vars1>0$.
			\end{enumerate}
		}
	\end{lemma}
	...
	\begin{definition}\label{def: finite element space}
		Suppose \edit{}{$\norm\cdot_q\lesssim \Norm\cdot$ (i.e. }$\F{U}\subset L^q(\Domain)$\edit{}{)} for some \edit{}{$q\in[1,\infty]$ and }connected, bounded domain $\Domain\subset\R^d$\edit{ with sufficiently smooth boundary}{}. We say that $\F M^k = \{\domain_1^k,\domain_2^k,\ldots\}$ is a \emph{mesh} if 
		$$\edit{\domain_i^k\subset}{\bigcup_{\domain\in\F M^k}\domain = } \Domain \qquad\text{and} \qquad |\edit{\domain_i^k\cap\domain^k_j}{\domain\cap\domain'}| = 0\qquad \text{for all \edit{$i$ and $j$}{$\domain,\domain'\in\F M^k,\ \domain\neq\domain'$}.}$$
		We say that $\tilde{\F{U}}^k\subset \F H$ is a \emph{finite element space} if there exists a mesh $\F M^k$ such that for all $\tilde{\var0}\in\tilde{\F{U}}^k$ \edit{there exists $\var0_i^k\in\tilde{\F{U}}^k$ such that
		...
		$$\forall\domain\in\F M^k \qquad \exists \var0|_\domain\in \tilde{\F{U}}^k \quad\text{such that}\quad \op{supp}(\var0|_\domain)\subset\bar{\domain}\quad\text{and}\quad\text{for a.e. }\vec{x}\in\domain,\ \tilde{\var0}(\vec{x})=\var0|_\domain(\vec{x}).$$
		}
		
		\edit{We}{Moreover, for $\meshsize\in(0,1)$, we} say that the sequence $(\tilde{\F{U}}^k)_{k\in\F N}$ is \emph{$\meshsize$-refining} if there exists \edit{a basis $\{e_1, \ldots,e_N\}\subset\tilde{\F{U}}^0\cap L^\infty(\Domain)$ such that for any $\var0_i^k\in\tilde{\F{U}}^k$ with $\supp(\var0_i^k)\subset\domain_i^k$ there exist $\vars0\in\R^{d\times d}$, $\vvars1\in\R^d$, $\vvars2\in\R^N$ such that 
		...
		$c_\alpha>0$ such that for each $\tilde{\var0}\in\tilde{\F{U}}^k$ and $\domain\in\F M^k$,
		$$ \exists (\var0,\alpha,\vec\beta)\in\tilde{\F U}^0\times\R^{d\times d}\times\R^d \quad \text{such that}\quad \op{det}(\alpha)\geq c_\alpha\meshsize^{kd} \quad\text{and}\quad \forall\vec{x}\in\Domain,\ \var0(\vec x) = \var0|_\domain(\alpha \vec{x}+\vec\beta). $$
		}
	...
	\end{lemma}
	This is precisely rewriting the statement of Definition~\ref{def: exp subspaces} into terms of resolution and approximation properties. The following theorem shows how to link $p$ and $q$ from Definition~\ref{def: finite element space} with $p'$ and $q'$ from Lemma~\ref{thm: pq to p'q'}.
	}

	\begin{theorem}\label{thm: generic a_U and a_E}
		Suppose $\F H= L^2(\Domain)$ for some connected, bounded domain $\Domain\subset\R^d$\edit{ with sufficiently smooth boundary}{} and $\norm\cdot_q\lesssim \Norm\cdot$ for some $q\in[1,\infty]$. \edit{If}{For $p\geq0$ and $\meshsize\in(0,1)$, if} $(\tilde{\F{U}}^k)_{k\in\F N}$ is a sequence of $\meshsize$-refining finite element spaces of order $p$, then \edit{}{$(\tilde{\var2}_k)_{k\in\F N}$ is an \aUaE\ for}
		\begin{align*}
			\aU &\leq \splitln{1}{\text{if }q\geq 2,}{\sqrt{\meshsize^{-d}}}{q<2 \edit{}{\text{ and }\sup_{\var0\in\tilde{\F{U}}^0}\frac{\norm{\var0}_\infty}{\norm{\var0}}<\infty},}
			& \aE &\geq \begin{cases}
				\meshsize^{-2p} & \text{if }\op{E} \text{ is $\Norm\cdot$-smooth at }\var0^*,
	...
	\end{theorem}
	The proof of this theorem is in Appendix~\ref{app: generic a_U and a_E}. \edit{}{Note that $\sup_{\var0\in\tilde{\F{U}}^0}\frac{\norm{\var0}_\infty}{\norm{\var0}}$ is finite whenever $\tilde{\F{U}}^0\subset L^\infty(\Domain)$ is finite dimensional, so this is not a very strong assumption.} The main take-home for this theorem is that the computation of $\aU$ and $\aE$ is typically very simple and clear given a particular choice of $\Norm\cdot$ and $\op{E}$. \edit{The only non-trivial case is when $\op{g}$ is not smooth enough to estimate directly. So long as $\tilde{\F{U}}^k$ are sufficiently dense in sublevel sets of $\op{g}$, the smoothness of $\op{f}$ is sufficient to give a convergence rate.}{}
	...
	For the other term, note that for all $\vec{r}\in\R^m$, denote $\varphi\coloneqq\A^*\vec{r}$, then 
	\begin{multline}
		...
		= \sum_{\domain\in\F M^k}|\domain|^{-1}\iint_\domain [\varphi(\vec{x}) - \varphi(\vec{y})] \diff\vec{x}\diff\var0^*(\vec{y}) 
		\\\leq \max_{\domain\in\F M^k}\op{diam}(\domain)\norm{\nabla\varphi}_\infty\Norm{\var0^*}.
	\end{multline}
	...
	\end{equation}
	This last term we denote the semi-norm $|\A^*|_{\ell^2\to C^1}$ which is shown to be finite in Section~\ref{sec: smoothing operators}. For now, assuming that $|\A^*|_{\ell^2\to C^1}<\infty$, we conclude that $\op{E}(\tilde{\var2}_k)-\op{E}(\var0^*)\lesssim 2^{-k}$.}
	
	This computation confirms two things, firstly that the scaling constant is indeed $\aE = 2$, and secondly that the required smoothness to achieve a good rate with Algorithm~\ref{alg: refining FISTA} is that $\A^*\colon\R^m\to C^1(\Domain)$ is a bounded operator. This accounts for using the weaker topology of $\C M(\Domain)$ rather than $L^1(\Domain)$. In Section~\ref{sec: smoothing operators} we will show two practical examples where this seminorm is accurately computable. 
	...
	\edit[2]{}{First we recall the setting of Definition~\ref{def: finite element space}, fix: $p\geq0$, $q\in[1,\infty]$, $\meshsize\in(0,1)$, $N\in\F N$, connected and bounded domain $\Domain\subset\R^d$, and $\var0^*\in\argmin_{\var0\in\F U}\op{E}(\var0)$. We assume that $\F H=L^2(\Domain)$, $\norm{\cdot}_q\lesssim\Norm\cdot$, and there exist spaces $(\tilde{\F U}^k)_{k\in\F N}$ with $\tilde{\F{U}}^k\subset \F{U}\cap L^\infty(\Domain)$ containing a sequence $(\tilde{\var2}_k\in \tilde{\F U}^k)_{k\in\F N}$ such that $\Norm{\tilde{\var2}_k - \var0^*} \lesssim \meshsize^{kp}$. Furthermore, there exists constant $c_\alpha>0$ and finite partitions $\F M^k=\{\domain_1^k,\domain_2^k,\ldots\}$ of $\Domain$ such that:
	\begin{equation}\label{eq: decomposition property}
		\forall (\tilde{\var0},\domain)\in\tilde{\F{U}}^k\times\F M^k,\quad \exists (\var0,\alpha,\vec\beta)\in\tilde{\F U}^0\times\R^{d\times d}\times\R^d \quad \text{such that}\quad \op{det}(\alpha)\geq c_\alpha\meshsize^{kd} \quad\text{and}\quad \forall\vec{x}\in\Domain,\ \var0(\vec x) = \splitln{\tilde{\var0}(\alpha \vec{x}+\vec\beta)\qquad}{\text{if }\alpha \vec{x}+\vec\beta\in\domain,}{0}{\text{else.}}
	\end{equation}
	...
	\begin{lemma}\label{app:thm: specific a_U bound}
		...
		$$ \norm{\edit{\tilde{\tens{\Pi}}_k\var2}{\tilde{\var2}_k}} \lesssim \splitln{\edit{\Norm{\var2}}{1}}{\text{if }q\geq2, \text{ or}}{\edit{\sqrt{N}\Norm{\var2}}{}\meshsize^{-\frac{kd}{2}} \qquad}{\text{ if }1\leq q<2,\ \edit{}{\sup_{\var0\in\tilde{\F{U}}^0}\frac{\norm{\var0}_\infty}{\norm{\var0}}<\infty}.} \edit{}{\quad\text{in particular, }\quad q' = \splitln{0}{q\geq2}{-\frac{d}{2}}{1\leq q<2.}}$$
	\end{lemma}
	\begin{proof}
		...
		\begin{equation}
			\sup_{\tilde{\var0}\in\tilde{\F{U}}^k}\frac{\norm{\tilde{\var0}}_\infty}{\norm{\tilde{\var0}}} = \sup_{\tilde{\var0}\in\tilde{\F{U}}^k}\max_{\domain\in\F M^k} \frac{\norm{\tilde{\var0}}_{L^\infty(\domain)}}{\norm{\tilde{\var0}}_{L^2(\Domain)}} \leq \sup_{\tilde{\var0}\in\tilde{\F{U}}^k}\max_{\domain\in\F M^k} \frac{\norm{\tilde{\var0}}_{L^\infty(\domain)}}{\norm{\tilde{\var0}}_{L^2(\domain)}}.
		\end{equation}
		However, due to the decomposition property \eqref{eq: decomposition property}, for each $\domain\in\F M^k$ and $\tilde{\var0}\in\tilde{\F{U}}^k$ there exists $(\var0,\alpha,\vec\beta)\in\tilde{\F{U}}^0\times\R^{d\times d}\times\R^d$ such that
		\begin{equation}
			\norm{\var0}_\infty = \norm{\tilde{\var0}}_{L^\infty(\domain)},\qquad  \norm{\var0}^2 = \int_\Domain |\var0(\vec x)|^2\diff\vec{x} = \int_{\alpha\vec x+\vec\beta\in\domain} |\tilde{\var0}(\alpha\vec x+\vec\beta)|^2\diff\vec{x} = \op{det}(\alpha)^{-1}\norm{\tilde{\var0}}_{L^2(\domain)}^2.
		\end{equation}
		Combining these two equations confirms $\norm{\tilde{\var2}_k} \lesssim \sqrt{\op{det}(\alpha)^{-1}} \leq c_\alpha^{-\frac12}\meshsize^{-\frac{kd}{2}}$ as required.
		} 		
		\qed\end{proof}
	
	\begin{lemma}\label{app:thm: generic a_E bound}
		\edit[2]{Let $(\tilde{\F{U}}^k)_k$ be a sequence of $\meshsize$-refining finite element spaces of order $p$ with $\tilde{\var2}_k=\argmin_{\var0\in\tilde{\F{U}}^k}\op{E}(\var0)$.}{Suppose $\F H$ and $(\tilde{\F U}^k)_{k\in\F N}$ satisfy Definition~\ref{def: finite element space} and $\var0^*$ the minimiser of $E$ such that $\Norm{\tilde{\var2}_k-\var0^*}\lesssim \meshsize^{kp}$.}
		\begin{enumerate}
	...
	\end{lemma}
	\begin{proof}
		\edit{Each statement is}{Both statements are true} by definition, observe
		\begin{gather}

...
\usepackage{amsfonts,amssymb,mathtools,stmaryrd,dsfont,tikz,amsthm,bm}
...
The Fast Iterative Shrinkage-Thresholding Algorithm (FISTA) was proposed by \citet{Beck2009}
as an extension of Nesterov's fast gradient method (see~\cite{Nesterov2004})
and is now a very popular algorithm for minimising the sum of two convex functions.
...
\end{equation}
where $\func1\colon\F H\to\R$ is a convex differentiable function with $L$-Lipschitz gradient and $\func2\colon\F H\to\bar\R$ is a convex function, whose ``proximity operator'' is easy to compute.
The iterates of the FISTA algorithm will be denoted $\varf0_n\in\F H$. \citet{Beck2009} showed that $\Func0(\varf0_n) - \Func0(\varf0^*)$ converges at the optimum rate of $n^{-2}$.
% when $u^*\in \F H$ is finite dimensional and
\cite{Chambolle2015} showed (up to a minor modification) convergence of the iterates in
a general Hibert space setting.
% extended this result to the general Hilbert space setting.
Many further works have gone on to demonstrate %even
faster practical convergence rates for slightly modified variants of FISTA \citep{Tao2016,Liang2017,Alamo2019}.
...
\end{itemize}
\todo{There is much overlap between the techniques used in this work and those used in the literature of inexact optimisation, although we believe that our interpretation for the use outside of Hilbert spaces is the key novelty.} Some other works for FISTA-like algorithms include \citep{Jiang2012,Villa2013}. Of particular note, our stability estimate for FISTA  in \Cref{thm: exponential FISTA convergence} is very similar to \cite[Proposition 2]{Schmidt2011} and \cite[Proposition 3.3]{Aujol2015}. This is then used to analyse the convergence properties in our more general Banach space setting where all sources of inexactness come from subspace approximations. The ideas of \citet{Parpas2017} are similar although in application to the proximal gradient method with an additional smoothing on the functional $\func2$. The permitted refinement steps are also more broad in our work.


\subsection{Outline}
This chapter is organised as follows. \Cref{sec: prelims} defines notation and the generic form of our proposed refining FISTA algorithm, \Cref{alg: refining FISTA}. The main theoretical contribution of this work is the convergence analysis of \Cref{alg: refining FISTA} which is split into two parts: first we outline the proof structure in \Cref{sec: recipe}, then we state the specific results in the case of FISTA in \Cref{sec: FISTA convergence}. The main results are \Cref{thm: exponential FISTA convergence} and \Cref{thm: practical refinement criteria} which extend the convergence of FISTA to infinite dimensional Banach spaces with uniform/adaptively chosen subspaces $\spcf0^n$ respectively.

\Cref{sec: general examples} presents some general results for the application of \Cref{alg: refining FISTA} and \Cref{sec: Lasso definition} gives a much more detailed discussion of adaptive refinement for Lasso minimisation. In particular, we describe how to choose efficient refining discretisations to approximate $\varf0^*$, estimate the convergence of $\Func0$, and identify the support of $\varf0^*$. The numerical results in \Cref{sec: numerics} demonstrate these techniques in four different models demonstrating the apparent sharpness of our convergence rates and the computational efficiency of adaptive discretisations.


\section{Definitions and notation}\label{sec: prelims}
We consider optimisation of \eqref{eq: min F = fD+fC} over two spaces, the Banach space $(\spcf0,\Norm\cdot)$ and Hilbert space $(\F H,\IP\cdot\cdot,\norm\cdot)$, such that 
\begin{equation} 
...
\begin{equation}
	\Func0_0\colon\spcf0\cup\F H\to \R,\qquad \Func0_0(\varf0) \coloneqq \Func0(\varf0)-\Func0(\varf0^*).
\end{equation}
Note that the value of $\Func0_0$ is uniquely defined, even if $\varf0^*$ may not be. \todo{We could just say $E_0=E$ without loss of generality, just like $L=1$.}

The proposed generalised FISTA algorithm is stated in \Cref{alg: refining FISTA} for an arbitrary choice of refining subspaces $\spcf0^n\subset \spcf0\cap\F H$ for $n=0,1,\ldots$. The only difference is that on iteration $n$, all computations are performed in the subspace $\spcf0^n$. If $\spcf0^n=\spcf0=\F H$ then we recover the standard FISTA algorithm.

Without loss of generality we will assume $L=1$, i.e. $\nabla\func1$ is 1-Lipschitz. To get the general statement of any of the results which follow, replace $\Func0$ with $\frac{\Func0}{L}$. A more important distinction is to say that the properties of $\func1$ and $\func2$ are stated with respect to the Hilbert norm. In particular,
\begin{equation}
...
One defining property of the FISTA algorithm is an appropriate choice of inertia, dictated by $\vart1_n$. In particular, we will say that $(\vart1_n)_{n=0}^\infty$ is a \emph{FISTA stepsize} if
...
This definition is non-standard as we view $(\spcf0^n)^*$ embedded in $\spcf0$. Note that $\spcf0^n\subset\F H$ implies that $(\spcf0^n)^*\supset\F H$, therefore $\Pi_n$ is an extension of the classical orthogonal projection. It is convenient to allow an extension here because $\spcf0$ may be bigger than $\F H$, $(\spcf0^n)^*$ is the largest space such that $\Pi_n$ is sufficiently well-defined. Beyond this, the Hahn-Banach theorem may allow the domain of $\Pi_n$ to be extended further, but the definition is no longer unique. For example, in 1D the value of $\IP{\delta_0}{\frac{1}{\sqrt[4]{\varx0}}}$ is not well-defined, but the value of $\IP{\delta_0}{\1_{[0,1]}}$ can be chosen to be consistent with some sequence of molifiers. To account for this possibility, we shall state that the domain of $\Pi_n$ is $\bar{(\spcf0^n)^*}$, with the closure computed in an appropriate topology dictated by $\Norm\cdot$.
...
\begin{algorithm}\caption{Refining sub-space FISTA}\label{alg: refining FISTA}
	\centering
	\begin{algorithmic}[1]
...
In this section we give an intuitive outline of the full proof for convergence of \cref{alg: refining FISTA} before giving formal theorems and proofs in the next section. First we recall the classical FISTA convergence guarantee given by \citet[Theorem 2]{Chambolle2015}:
...
\begin{adaptiveStep}\label{step: step 1 stability}
	The first step is to generalise \eqref{eq: classical FISTA} to account for the adapting subspaces $\spcf0^n$. In the notation of \cref{alg: refining FISTA}, \cref{thm: mini FISTA convergence} shows that 
	\begin{equation}\label{eq: mini FISTA convergence}
		\vart1_N^2\Func0_0(\varf0_N) + \sum_{n=1}^{N-1}\rho_n \Func0_0(\varf0_n) + \tfrac12\norm{\varf1_N-\varf2_N}^2 \leq \tfrac12\norm{\varf0_0-\varf2_0}^2 +\tfrac{\norm{\varf2_N}^2-\norm{\varf2_0}^2}2 + \sum^N_{n=1} \vart1_n\Func0_0(\varf2_n) + \IP{\varf1_{n-1}}{\varf2_{n-1}-\varf2_n}
	\end{equation}
	for $\varf2_n = \argmin_{\varf0\in\spcf0^n}\Func0(\varf0)$. The similarities to \eqref{eq: classical FISTA} are clear. If $\spcf0^n=\spcf0$, then we have $\varf2_n=\varf0^*$ and the two estimates agree. These extra terms quantify the robustness to changing of discretisation.
\end{adaptiveStep}


\paragraph{Step 2: Quantifying the scaling properties}
\begin{adaptiveStep}\label{step: step 2 smoothness}
	To show that the extra terms in \eqref{eq: mini FISTA convergence} are small, we need to quantify the approximation properties of $\spcf0^n$. To do so, we construct a secondary sequence $n_0 < n_1 < \ldots$ and constants $\vars3_{\Varf0},\vars3_{\Func0}\geq1$ such that for each $k\in\F N$
	\begin{equation}
		n\leq n_k \implies \norm{\varf2_n} \lesssim \vars3_{\Varf0}^k, \qquad n\geq n_k \implies \Func0_0(\varf2_n) \lesssim \vars3_{\Func0}^{-k}.
	\end{equation}
	The choice of exponential scaling is very natural if we consider the $\spcf0^n$ to be the subspace of functions discretised on a uniform mesh. If that mesh sequentially refined, then the resolution of the mesh will be of order $h^k$ after $k$ refinements and for some $h<1$. The integer $n_k$ should then be the time when the mesh has refined $k$ times.
	
	The value of $\vars3_{\Varf0}$ is dictated by the smoothness of the norm of $\spcf0$ with respect to that of $\F H$. If $\norm{\varf0^*}<\infty$, then the value is $\vars3_{\Varf0}=1$, otherwise there is some damage to the rate of convergence generated by this factor. On the other hand, the value of $\vars3_{\Func0}$ is dictated by the smoothness of $\Func0$ at the point $\varf0^*$. If an efficient basis is used, then $\vars3_{\Func0}$ is large. The trade-off between $\vars3_{\Func0}$ and $\vars3_{\Varf0}$ dictates the final convergence rate of the algorithm.
\end{adaptiveStep}

\paragraph{Step 3: Generalising the convergence bound}
\begin{adaptiveStep}\label{step: step 3 convergence bound}
	In this step we combine the FISTA stability estimate with the subspace approximation guarantees to provide a sharper estimate of stability with respect to the parameters $\vars3_{\Func0}$ and $\vars3_{\Varf0}$. For example, if $\spcf0^n$ is of the form 
	$$ \spcf0^{n_k} = \spcf0^{n_k+1} = \ldots = \spcf0^{n_{k+1}-1},$$
	then many terms on the right-hand side of \eqref{eq: mini FISTA convergence} telescope to 0. The result of this is presented in \Cref{thm: mini exponential FISTA convergence} but the moral is that the stability error in \eqref{eq: mini FISTA convergence} should scale in proportion to the $k$ (related to the resolution) rather than the total number of iterations $N$.
\end{adaptiveStep}

\paragraph{Step 4: Sufficiently fast refinement}
\begin{adaptiveStep}\label{step: step 4 sufficiently fast}
	In \Cref{step: step 3 convergence bound} we developed a convergence bound, now we wish to show that it is only worse than the classical \eqref{eq: classical FISTA} by a constant factor. In particular, it is equivalent to run \Cref{alg: refining FISTA} for $N$ iterations or the classical FISTA algorithm for $N$ iterations on the fixed subspace $\spcf0^N$. The estimate from \eqref{eq: classical FISTA} provides the estimate $N^2\Func0_0(\varf0_N)\lesssim \norm{\varf0_0-\varf2_N}^2 = O(\vars3_{\Varf0}^{2K})$ for $N\leq n_K$. \Cref{thm: sufficiently fast} shows that \Cref{alg: refining FISTA} can achieve the same order of approximation so long as $\spcf0^n$ refine sufficiently quickly, i.e. $n_k$ are sufficiently small.
\end{adaptiveStep}

\paragraph{Step 5: Sufficiently slow refinement}
\begin{adaptiveStep}\label{step: step 5 sufficiently slow}
	The result of \Cref{step: step 4 sufficiently fast} is sufficient to prove convergence, but not yet a rate. If the subspaces refine too quickly, then the influence of $\norm{\varf0^*}=\infty$ will slow the rate of convergence. If the resolution is too coarse then we are overfitting to the discrete minimiser, but if the resolution is too fine then convergence of the energy is slow. \Cref{thm: sufficiently slow} balances these two factors in an optimal way for \cref{alg: refining FISTA} resulting in a convergence rate of 
	$$ \Func0_0(\varf0_N) \lesssim \frac{\vars3_{\Varf0}^{2K}}{N^2} \lesssim \frac{N^{2\kappa}}{N^2}$$
	for all $N\in\F N$, some $\kappa\in[0,1)$ depending on $\vars3_{\Varf0},\vars3_{\Func0}$. In particular, if $\varf0^*\in\F H$ then we recover the classical rate with $\kappa=0$.
\end{adaptiveStep}
...
\begin{adaptiveStep}\label{step: step 6 adaptivity}
	...
	Spatial adaptivity is robust, so long as the scaling properties of \Cref{step: step 2 smoothness} are satisfied. The only other constraint is to ensure that the partial telescoping of \Cref{step: step 3 convergence bound} still holds. \Cref{thm: mini exponential FISTA convergence} shows that a sufficient condition is
	...
	Adaptively choosing the time of refinement is more challenging due to the non-descent property of FISTA. The idea is that $\spcf0^n$ should be refined as soon as the value $\Func0(\varf0_n)$ is sufficiently close to $\Func0(\varf2_n)$. This is analysed in \Cref{thm: stronger exponential FISTA convergence} which shows convergence results of the form
	$$ \min_{n\leq N}\Func0_0(\varf0_n) \lesssim \frac{N^{2\kappa}}{N^2}$$
	for all $N\in\F N$, with the same $\kappa\in[0,1)$ from \Cref{step: step 5 sufficiently slow}. The penalty for accelerating the refinement time is a potential loss of stability in $\Func0(\varf0_n)$, however, the asymptotic rate is equivalent and this behaviour has not been seen in numerical experiments.
\end{adaptiveStep}

\section{Proof of convergence}\label{sec: FISTA convergence}
In this section we follow the recipe motivated in \Cref{sec: recipe} to prove convergence of two variants of \Cref{alg: refining FISTA}. Each of the main theorems and lemmas will be stated with a sketch proof in this section. The details of the proofs are either trivial or very technical and are therefore placed in \Cref{app: FISTA convergence} to preserve the flow of the argument.

\subsection{Computing the convergence bound}
For \Cref{step: step 1 stability} of \Cref{sec: recipe} we look to replicate the classical bound of the form in \eqref{eq: classical FISTA} for \Cref{alg: refining FISTA}. The proofs in this step follow the classical arguments of \citet{Beck2009,Chambolle2015} very closely. 

\subsubsection{Single iterations}
We first wish to understand a single iteration of \Cref{alg: refining FISTA}. This is done through the following two lemmas.
\begin{lemma}[equivalent to {\cite[Lemma 1]{Chambolle2015}}]\label{thm: descent lemma}
...
\end{lemma}
\noindent This is exactly the result of \citet{Chambolle2015} applied to the function $\varf0\mapsto\Func0(\Pi_n\varf0)$. Applying \Cref{thm: descent lemma} to the iterates from \Cref{alg: refining FISTA} gives a more explicit inequality.

\begin{lemma}[analogous to {\cite[Theorem 2]{Chambolle2015}, \cite[Theorem 1]{Beck2009}}]\label{thm: one step FISTA}
	\Copy{thm: one step FISTA}{
		Let $\spcf0^n\subset\F H\cap\spcf0$ and $\varf2_n\in\spcf0^n$ be chosen arbitrarily and $\varf0_n$/$\varf1_n$ be generated by \Cref{alg: refining FISTA} for all $n\in\F N$.
	...
	\end{equation}
\end{lemma}
\noindent The proof is given in \cref{app:thm: one step FISTA} and is a result of the convexity of $\Func0$ for a well chosen $\varf2$ in \Cref{thm: descent lemma}. 

\subsubsection{Generic convergence bound}
\Cref{thm: one step FISTA} gives us an understanding of a single iteration of \Cref{alg: refining FISTA}, summing over $n$ then gives our generic convergence bound for any variant of \Cref{alg: refining FISTA}.

\begin{theorem}\label{thm: mini FISTA convergence}
	\Copy{thm: mini FISTA convergence}{
		Fix a sequence of subspaces $\{\spcf0^n\subset\spcf0\cap\F H\st n\in\F N\}$, arbitrary $\varf0_0\in \spcf0^0$, and FISTA stepsize choice $(\vart1_n)_{n\in\F N}$. Let $\varf0_n$ and $\varf1_n$ be generated by \Cref{alg: refining FISTA}. 
		Then, for any choice of $\varf2_n\in\spcf0^n$ and $N\in\F N$ we have}
	\begin{equation}\label{eq: FISTA inequality}
	...
	\end{equation}
\end{theorem}
\noindent The proof is given in \cref{app:thm: mini FISTA convergence}. This result is the key approximation for showing convergence of FISTA with refining sub-spaces. In the classical setting, we have $\spcf0^n=\spcf0=\F H$, $\varf2_n=\varf0^*$ and the extra terms on the right-hand side collapse to 0. 

The natural choice in \eqref{eq: FISTA inequality} is $\varf2_n=\Pi_n\varf0^*$, however, there are simple counter-examples which give $\Func0(\Pi_n\varf0^*)=\infty$ and so this inequality becomes useless. A simple example, if $\func1(\varf0) = \norm{\varf0}_2^2$, $\func2$ is the indicator on the set $\F D=\{\varf0\in L^1([0,1])\st \varf0(\varx0) \geq \varx0\}$ and $\spcf0^n$ is a set of piecewise constant functions, then $\varf0^*=\varx0\mapsto \varx0$. On the other hand, suppose one of the pixels of the discretisation is $[\varx0_0-\meshsize,\varx0_0+\meshsize]$, then 
$$\Pi_n\varf0^*\left(\varx0_0+\sfrac\meshsize2\right) = \argmin_{\vars5\in\R}\int_{\varx0_0-\meshsize}^{\varx0_0+\meshsize}(\varf0^*(\varx0)-\vars5)^2d\varx0 = \argmin_{\vars5\in\R}\int_{\varx0_0-\meshsize}^{\varx0_0+\meshsize}(\varx0-\vars5)^2d\varx0 = \varx0_0 < \varx0_0+\sfrac\meshsize2.$$
In particular $\Pi_n\varf0^* \notin\F D$ therefore $\Func0(\Pi_n\varf0^*) = \infty$.

The choice $\varf2_n=\argmin_{\varf0\in\spcf0^n}\Func0(\varf0)$ is much more robust and allows us to apply \cref{alg: refining FISTA} more broadly. The penalty for this flexibility is a more complicated analysis, each time the sub-space changes, the system receives a `shock' proportional to $\norm{\varf2_{n}-\varf2_{n+1}}$. 


\subsection{Convergence bound with milestones}
In standard FISTA, the right-hand side of \eqref{eq: FISTA inequality} is a constant. The following lemma minimises the growth of the `constant' as a function of $N$ by partially telescoping the sum on the right-hand side. Before progressing to the content of \cref{step: step 3 convergence bound}, we will first formalise the definition of the constants $\vars3_{\Varf0}$ and $\vars3_{\Func0}$ introduced in \Cref{step: step 2 smoothness}.
...
\end{definition}
In this section we will simply assume that such sequences exist and in \Cref{sec: general examples} we will give some more general examples. 

\begin{lemma}\label{thm: mini exponential FISTA convergence}
	\Copy{thm: mini exponential FISTA convergence}{
		Let $\varf0_n$, $\varf1_n$ be generated by \Cref{alg: refining FISTA}, $(n_k\in\F N)_{k=0}^\infty$ be a monotone increasing sequence, and choose
...
\end{lemma}
\noindent The proof is given in \cref{app:thm: mini exponential FISTA convergence}. The introduction of $n_k$ has greatly compressed the expression of \Cref{thm: mini FISTA convergence}. On the right-hand side, we now only consider $\Func0_0$ evaluated on the sequence $\tilde{\varf2}_k$ and there are $K$ elements to the sum rather than $N$.

\subsection{Refinement without overfitting}
The aim of \cref{step: step 4 sufficiently fast} is to show that $n$ iterations of \cref{alg: refining FISTA} is no slower (up to a constant factor) than $n$ iterations of classical FISTA on the space $\spcf0^n$. In other words, we would like to ensure that
\begin{equation}
	\Func0(\varf0_n) - \min_{\varf0\in\spcf0^n}\Func0(\varf0) \lesssim \frac{\norm{\varf0_0-\argmin_{\varf0\in\spcf0^n}\Func0(\varf0)}^2}{n^2}
\end{equation}
uniformly in $n$. If this condition is not satisfied, then it indicates that computational effort has been wasted by a poor choice of subspaces. This can be interpreted as an overfitting to the discretisation of $\Func0|_{\spcf0^n}$ rather than the desired function $\Func0|_{\spcf0}$. Combining the assumptions given by \Cref{def: exp subspaces} and the result of \cref{thm: mini exponential FISTA convergence}, the following lemma proves the convergence of \cref{alg: refining FISTA} provided that the refinement times $n_k$ are sufficiently small (i.e. $\spcf0^n$ grows sufficiently quickly).

\begin{lemma}\label{thm: sufficiently fast}
	\Copy{thm: sufficiently fast}{
		Suppose $\spcf0^n, \varf0_n, \varf1_n$ and $n_k$ satisfy the conditions of \Cref{thm: mini exponential FISTA convergence} and $\{\tilde{\spcf0}^k\}$ forms an $(\vars3_{\Varf0},\vars3_{\Func0})$-discretisation for $\Func0$ with
		$$\tilde{\varf2}_k\in \left[\argmin_{\varf0\in\tilde{\spcf0}^k}\Func0(\varf0)\right]\cap\spcf0^{n_k}\cap\spcf0^{n_k+1}\cap\ldots\cap\spcf0^{n_{k+1}-1}.$$
		If either:
		\begin{itemize}
		...
		\end{itemize}
		then
		$$ \Func0_0(\varf0_N) \lesssim \frac{\vars3_{\Varf0}^{2K}}{N^2}\qquad \text{for all}\qquad n_K\leq N< n_{K+1}.$$
		}
\end{lemma}

\noindent The proof is given in \cref{app:thm: sufficiently fast}. We make two observations of the optimality of \Cref{thm: sufficiently fast}:
\begin{itemize}
	\item The convergence guarantee for $N$ iterations of classical FISTA in the space $\spcf0^N$ is 
	$$ \Func0_0(\varf0_N) \lesssim \frac{\norm{\varf0_0-\argmin_{\varf0\in\spcf0^N}\Func0(\varf0)}^2}{N^2} + \min_{\varf0\in\spcf0^N}\Func0_0(\varf0) \lesssim \frac{\vars3_{\Varf0}^{2K}}{N^2} + \vars3_{\Func0}^{-K}.$$
	This is equivalent to \Cref{thm: sufficiently fast} after the assumptions on $n_k$.
...
In \Cref{thm: sufficiently fast} we show that $\Func0(\varf0_n)$ converges at a rate depending on $k$ and $n$ so long as $k$ grows sufficiently quickly. On the other hand, as $k$ grows, the rate becomes worse and so we need to also put a lower limit on the growth of $n_k$. The following lemma completes \cref{step: step 5 sufficiently slow} by computing the global convergence rate of $\Func0_0(\varf0_n)$ when $k$ grows at the minimum rate which is consistent with \Cref{thm: sufficiently fast}.

As a special case, note that if $\vars3_{\Varf0}=1$ then \Cref{thm: sufficiently fast} already gives the optimal $O(N^{-2})$ convergence rate. This is in fact a special case of \citet{Aujol2015}. If $\varf0^*\in\F H$, then it is not possible to refine `too quickly' and the following lemma is not needed.
...
\begin{lemma}\label{thm: sufficiently slow}
	\Copy{thm: sufficiently slow}{
		...
		then 
		$$\Func0_0(\varf0_N) \lesssim \frac{1}{N^{2(1-\kappa)}}\qquad \text{ where } \qquad \kappa = \frac{\log \vars3_{\Varf0}^2}{\log \vars3_{\Func0} + \log \vars3_{\Varf0}^2}.$$}
\end{lemma}
\noindent The proof is given in \cref{app:thm: sufficiently slow}.

\subsubsection{FISTA convergence with a priori discretisation}
We can summarise \Cref{thm: mini exponential FISTA convergence,thm: sufficiently fast,thm: sufficiently slow} into a single theorem stating the convergence guarantees when $\spcf0^n$ and $n_k$ are chosen a priori.
\begin{theorem}\label{thm: exponential FISTA convergence}
	\Copy{thm: exponential FISTA convergence}{
		...
		$$ \tilde{\spcf0}^k = \spcf0^{n_k}, \qquad \tilde{\varf2}_k \in \left[\argmin_{\varf0\in\tilde{\spcf0}^k}\Func0(\varf0)\right]\cap\spcf0^{n_k+1} \cap\ldots\cap \spcf0^{n_{k+1}-1}$$
		for all $k\in\F N$. Compute $\varf0_n$, $\varf1_n$ by \Cref{alg: refining FISTA}.
		...
		\end{itemize}
		then
		$$\Func0_0(\varf0_N) \lesssim \frac{1}{N^{2(1-\kappa)}}\qquad \text{ where } \qquad \kappa = \frac{\log \vars3_{\Varf0}^2}{\log \vars3_{\Func0} + \log \vars3_{\Varf0}^2} \qquad \text{uniformly for } N\in\F N.$$
		}
\end{theorem}
\noindent This theorem is very easy to implement numerically and requires very little knowledge of how to estimate $\Func0_0(\varf0_n)$. So long as $\vars3_{\Varf0}$ and $\vars3_{\Func0}$ can be computed analytically, choosing $\tilde{\spcf0}^k$ to be uniform discretisations and $\tilde{\spcf0}^k = \spcf0^{n_k}=\ldots= \spcf0^{n_{k+1}-1}$ will give the stated convergence rate.
...
\Cref{thm: sufficiently fast} gives a sufficient condition on $n_k$ for converging at the rate $O(N^{2(\kappa-1)})$ but it is not necessary. Indeed for $n\leq n_k$ the result shows 
$$\Func0_0(\varf0_n) \geq \min_{\varf0\in\spcf0^n}\Func0_0(\varf0) = O(\vars3_{\Func0}^{-k}) = O(n^{2(\kappa-1)})$$
which suggests that to converge faster than $n^{2(\kappa-1)}$ requires choosing smaller $n_k$. As an example, in \Cref{sec: wavelet examples} we will see \cref{alg: refining FISTA} can converge at a linear rate, although this is not possible without adaptive refinement times. On the other hand, choice of spatial adaptivity has no impact on rate but can impact computational efficiency. It will be permitted to use greedy discretisation techniques so long as each collection $\{\spcf0^n \st n_k\leq n<n_{k+1}\}$ satisfies a monotonicity condition.

While \Cref{thm: exponential FISTA convergence} also allows for spatial adaptivity, we will simplify the requirements to a form which seems more practical for implementation and also allows for time adaptivity. We make the assumption that the choice of $n_k$ depends only on the value $\Func0(u^{n_k-1})$ and therefore make $u^{n_k-1}$ the root of our requirements. \cref{thm: sufficiently fast} suggests that a good refinement time strategy is to choose $n_k$ to be the minimal integer such that $\Func0_0(\varf0^{n_k-1})\lesssim \vars3_{\Func0}^{-k}$. However, the value of $\Func0_0$ may be hard to estimate and so we introduce a `backstop' condition which guarantees that convergence is no slower than the rate given by \Cref{thm: exponential FISTA convergence}. The only constraint on spatial adaptivity is the piecewise monotone inclusion and that $\spcf0^{n_k} \ni u^{n_k-1}$. This is confirmed by the following theorem.
\begin{theorem}\label{thm: stronger exponential FISTA convergence}
	\Copy{thm: stronger exponential FISTA convergence}{
		...
		for all $k\in\F N$.
		Compute $\varf0_n$, $\varf1_n$ by \Cref{alg: refining FISTA}.
		...
		$$\min_{n\leq N}\Func0_0(\varf0_n) \lesssim \frac{1}{N^{2(1-\kappa)}}\qquad \text{ where } \qquad \kappa = \frac{\log \vars3_{\Varf0}^2}{\log \vars3_{\Func0} + \log \vars3_{\Varf0}^2}$$
		uniformly for $N\in\F N$.
	}
\end{theorem}
\noindent The proof is given in \cref{app:thm: stronger exponential FISTA convergence}. If we directly compare \Cref{thm: exponential FISTA convergence,thm: stronger exponential FISTA convergence}, we note that the convergence rate is the same in both theorems but the price for better adaptivity is a slightly weaker stability guarantee (i.e. the addition of the min on the left-hand side). In \Cref{thm: exponential FISTA convergence}, as in the original FISTA algorithm, the sequence $\Func0(\varf0_n)$ is not monotone but the magnitude of oscillation is guaranteed to decay in time. This behaviour is boundedness is lost in \cref{thm: stronger exponential FISTA convergence} although it still holds on a subsequence. Although we do not prove it here, it can be shown that the stronger condition 
\begin{equation}
	\tilde{\varf2}_k \in \left[\argmin_{\varf0\in\tilde{\spcf0}^k}\Func0(\varf0)\right]\cap\spcf0^{n_k+1} \cap\ldots\cap \spcf0^{n_{k+1}-1}\cap\ldots\cap \spcf0^N
\end{equation}
is sufficient to restore the stronger guarantee on $\Func0_0(\varf0_n)$. Again, monotonicity of $\spcf0^n$ corresponds with improved stability of \cref{alg: refining FISTA}.

To enable more practical implementation of \Cref{thm: stronger exponential FISTA convergence}, the following lemma describes several refinement time strategies which provide sufficient condition for $\Func0_0(\varf0_n) \lesssim \vars3_{\Func0}^{-k}$.
...
\noindent The proof is given in \cref{app:thm: practical refinement criteria}. The refinement criteria described by \Cref{thm: practical refinement criteria} can be split into two groups. Cases (1), (3), and (4) justify that any choice of $\tilde{\spcf0}^k\ni\varf0_k$ (or $\varf0_{n_k-1}$ in \Cref{thm: stronger exponential FISTA convergence}) satisfies the required conditions. In cases (2) and (5), $\varf0_k$ is sufficient to choose the refinement time $n_k$ but not the subspace $\tilde{\spcf0}^k$. In these cases one could, for example, choose $\tilde{\spcf0}^k$ to be a uniform discretisation with apriori estimates.
...
In this work we consider the Lasso problem to be both a motivating example and source of numerical examples, however, \Cref{alg: refining FISTA} is much more broadly applicable. Here we justify this claim by demonstrating that the constants $\vars3_{\Varf0}$ and $\vars3_{\Func0}$ can be easily computed in many examples. 

The cases where $\spcf0$ is finite or countable dimensional are quite straight forward. If the total number of refinements is finite (i.e. $\spcf0^n = \spcf0^N$ for all $n\geq N$, some $N\in\F N$) then $\vars3_{\Varf0} = 1$. This holds for most finite dimensional problems as well as the countable Lasso example discussed in detail in \cref{sec: Lasso definition}. The examples we will consider here are where $\F H = L^2(\Domain)$ for some compact domain $\Domain\subset\R^d$, and $\{\tilde{\spcf0}^n\st n\in\F N\}$ are finite dimensional finite-element--like spaces. We will now clarify exactly what is meant by these properties and then demonstrate that they are sufficient to find simple formulae for the values $\vars3_{\Varf0}$ and $\vars3_{\Func0}$.
...
\begin{definition}
	...
	$\min_{\varf2\in\tilde{\spcf0}^k}\norm{\varf2-\varf0^*}_{W^{0,q}} \lesssim \meshsize^{kp}\norm{\varf0^*}_{W^{p,q}}$ \citep{Strang1972}.
\end{definition}
We note that any piecewise polynomial finite element space can be used to form a $\meshsize$-refining sequence of subspaces. Wavelets with a compactly supported basis satisfy this definition but it is hard to specify the value $N$ in general. Similarly, a Fourier basis does satisfy the scaling properties but each basis vector has global support. Both of these exceptions are important and could be accounted for with further analysis but we focus on the more standard finite element case. The following theorem shows that all finite element spaces achieve the same basic rates of convergence in \Cref{alg: refining FISTA}, depending on the particular properties of $\Norm\cdot$ and $\Func0$.
...
\end{theorem}
The proof of this theorem is in \Cref{app: generic a_U and a_E}. The main take-home for this theorem is that the computation of $\vars3_{\Varf0}$ and $\vars3_{\Func0}$ is typically very simple and clear given a particular choice of $\Norm\cdot$ and $\Func0$. The only non-trivial case is when $\func2$ is not smooth enough to estimate directly. So long as $\tilde{\spcf0}^k$ are sufficiently dense in sublevel sets of $\func2$, the smoothness of $\func1$ is sufficient to give a convergence rate.
...
The aim of this section is to develop all of the necessary tools for implementing \Cref{alg: refining FISTA} on the energy \eqref{eq: Lasso energy} using the convergence guarantees of either \Cref{thm: exponential FISTA convergence} or \Cref{thm: stronger exponential FISTA convergence}. This includes computing the rates $\vars3_{\Varf0}$ and $\vars3_{\Func0}$, estimating the continuous gap $\Func0_0(\varf0_n)$, and developing an efficient refinement choice for $\spcf0^n$.
...
Application of \Cref{thm: generic a_U and a_E} gives $\vars3_{\Varf0}=2^{\frac d2}$ although although $\norm\cdot_1$ is too strong a metric and results in $\vars3_{\Func0}=2^0$. This can be seen because for any $\varf0\in L^1(\Domain^d)$, 
\begin{equation}
	\norm{\varf0-\delta_0}_1 = \sup_{\vard0\in C(\Domain^d), \norm{\vard0}_\infty\leq 1} \IP{\vard0}{\varf0-\delta_0} = \norm{\varf0}_1+\norm{\delta_0}_1 \geq 1.
\end{equation}
To improve our estimate of $\vars3_{\Func0}$ requires additional assumptions on $\A$. For any $\varf2\in\tilde{\spcf0}^k$ such that $\norm{\varf2}_1\leq \norm{\varf0^*}_1$, we compute
\begin{align}
...
\end{align}
Choosing $\varf2 = \tilde\Pi_k\varf0^*$ for any bounded linear extension to the orthogonal projection $\tilde\Pi_k\colon\spcf0\to\tilde{\spcf0}^k$ gives
...
This computation confirms two things, firstly that the scaling constant is indeed $\vars3_{\Func0} = 2$, and secondly that the required smoothness to achieve a good rate with \Cref{alg: refining FISTA} is that $\A^*\colon\R^m\to C^1(\Domain)$ is a bounded operator. This accounts for using the weaker topology of $\C M(\Domain)$ rather than $L^1(\Domain)$. In \Cref{sec: smoothing operators} we will show two practical examples were this (semi)norm is accurately computable. 

Inserting the computed rates into \Cref{thm: exponential FISTA convergence} or \Cref{thm: stronger exponential FISTA convergence} gives the guaranteed convergence rate
\begin{equation}\label{eq: Lasso energy rate}
	\kappa = \frac{\log \vars3_{\Varf0}^2}{\log \vars3_{\Func0} + \log \vars3_{\Varf0}^2} = \frac{d}{1+d} \quad \implies \quad \Func0_0(\varf0_n) \lesssim n^{-2(1-\kappa)} = n^{-\frac{2}{1+d}}.
\end{equation}
This rate can be used to infer the required resolution at each iteration, in particular on iteration $n$ with $n^2\simeq (\vars3_{\Func0}\vars3_{\Varf0}^2)^k$ we expect the resolution to be
\begin{equation}\label{eq: Lasso resolution rate}
	2^{-k} = \left(\vars3_{\Func0}\vars3_{\Varf0}^2\right)^{\frac{k}{1+d}} \simeq n^{-\frac{2}{1+d}}.
\end{equation}
...
$$\tilde{\spcf0}^k = \{\varf0\in \ell^1(\R) \st i\notin J_k\implies \varf0_i=0\}$$ for some sets $J_k\subset\F N$, i.e. infinite vectors with finitely many non-zeros. The key change in analysis from the continuous case is $\varf0^*\in\ell^2(\R)$, which leads to $\vars3_{\Varf0}=1$ and the expected optimal rate of $n^{-2}$, independent of $\vars3_{\Func0}$ or any additional properties of $\A$. The number of refinements will also be finite, therefore $n_k=\infty$ for some $k$ and the remaining conditions of \Cref{thm: exponential FISTA convergence,thm: stronger exponential FISTA convergence} hold trivially.

\subsection{Refinement metrics}\label{sec: Lasso gap and gradient}
\Cref{thm: practical refinement criteria} shows that adaptive refinement can be performed based on estimates of the function gap or the gradient. In this subsection we provide estimates for these values which can be easily computed.
...
As $\Norm\cdot = \norm\cdot_1$, the natural metric for $\partial_n\Func0$ is $\Norm\cdot_* = \norm\cdot_\infty$ which we can estimate
\begin{align}
	...
	\end{cases}
\end{align}
which can be used directly in \Cref{thm: practical refinement criteria}.

\paragraph{Discrete gap}
We now move on to the discrete gap, $\Func0(\varf0_n)-\min_{\varf0\in\spcf0^n}\Func0(\varf0)$. This can be computed with a dual representation, such as that used by \citet{Duval2017a},
\begin{align}
	...
	\\&= \max_{\vvard0\in\R^m} \splitln{-\ip{\vdata0}{\vvard0} -\tfrac12\norm{\vvard0}_{\ell^2}^2}{\qquad\Norm{\Pi_n\A^*\vvard0}_*\leq \mu}{-\infty}{\qquad\text{else}}
...
In particular,
\begin{equation}
	\Func0(\varf0) - \min_{\varf0\in\spcf0^n}\Func0(\varf0) = \Func0(\varf0) + \min_{\vvard0\in\R^m\st \Norm{\Pi_n\A^*\vvard0}_*\leq \mu} \Func0^*(\vvard0) \leq \Func0(\varf0) + \Func0^*(\vvard0)
\end{equation}
...
\end{equation}
To apply \Cref{alg: refining FISTA}, we are assuming that both $\func1(\varf0_n) = \frac12\norm{\vvard0_n}_{\ell^2}^2$ and $\Pi_n\nabla\func1(\varf0_n) = \Pi_n\A^*\vvard0_n$ are easily computable, therefore $\vars2$ and $\Func0(\varf0_n) +\Func0^*(\vars2\vvard0_n)$ are also easy to compute.

\subsubsection{Bounds for countable functionals}
Extending the results of \Cref{sec: bound discrete} to $\spcf0=\ell^1(\R)$ is analytically very simple but computationally relies heavily on the specific choice of $\A$. The computations of gradients and gaps carry straight over replacing $\Pi_n$ with the identity and adding the sets $J_n\subset\F N$ which define $\spcf0^n = \{\varf0\in\ell^1\st i\notin J_n\implies \varf0_i=0\}$. In particular,
\begin{align}
...
In the countable case, the sets $J_n$ give a clear partition into known/unknown values in these definitions. For $i\in J_n$ the computation is the same as in \Cref{sec: bound discrete}, then for $i\notin J_n$ we know $[\varf0_n]_i=0$ which simplifies the remaining computations. This leads to:
\begin{align}
...
\end{align}
Both estimates only rely on an upper bound of $\max_{i\notin J_n} |[\A^*\vvard0_n]_i|$. One example computing this value is seen in \Cref{sec: wavelet examples}.

\subsubsection{Bounds for continuous functionals}\label{sec: bound continuous}
Finally we extend the results of \Cref{sec: bound discrete} to continuous Lasso. Similar to the countable case, the exact formulae can be written down immediately:
\begin{align}
...
\end{align}
Now, both estimates rely on cell-wise supremum norms of $\A^*\vvard0_n$ which we assume is sufficiently smooth. We will therefore use a cell-wise Taylor expansion to provide a simple and accurate estimate. For instance, let $\vvarx0_i$ be the midpoint of the square $\domain_i^n$, then 
...
Therefore,
\begin{equation}
	|[\A^*\vars2_0\vvard0_n](\vvarx0)|< \mu - \sqrt{2(\Func0(\varf0_n)+\Func0^*(\vars2_0\vvard0_n))}|\A^*|_{\ell^2\to L^\infty} \qquad \implies \qquad \varf0^*(\vvarx0)= 0.\label{eq: support equation}
\end{equation}
This equation is valid when $\vvarx0$ is either a continuous or countable index, the only distinction is to switch to $\ell^\infty$ in the norm of $\A^*$. To make the equivalent statement on the discretised problem, simply replace $\vars2_0$ with $\vars2$ and $\A^*$ with $\Pi_n\A^*$. There are two short observations on this formula:
\begin{itemize}
	\item The convergence guarantee from \Cref{thm: exponential FISTA convergence} is for the quantity $\Func0(\varf0_n)-\Func0(\varf0^*)$, the more relevant quantity here is $\Func0^*(\vars2_0\vvard0_n)+\Func0(\varf0^*)$ for which there is no proven rate.
	\item In \Cref{sec: continuous lasso rate}, $|\A^*|_{\ell^2\to C^1}<\infty$ was required to compute a rate of convergence, but only $|\A^*|_{\ell^2\to L^\infty}<\infty$ is needed to estimate the support.
\end{itemize}


\subsection{Operator norms}\label{sec: smoothing operators}
For numerical implementation of Lasso, we are required to accurately estimate several operator norms of $\A$. For $\func1$ to be 1-Lipschitz we must divide by $\norm{\A^*\A}$, and the adaptivity described in \Cref{sec: continuous lasso rate,sec: bound continuous,sec: support detection} requires the values of $|\A^*|_{\ell^2\to L^\infty}$, $|\A^*|_{\ell^2\to C^1}$, and $|\A^*|_{\ell^2\to C^2}$. The aim for this section is to provide estimates of these norms and seminorms for the numerical examples presented in \Cref{sec: numerics}.
...
\begin{proof}
	...
	as required.
\end{proof}
If $\norm{\A^*\A}$ is not analytically tractable, then \Cref{thm: norm bound L2} enables it to be computed using standard finite dimensional methods. The operator $\A\A^*$ is always finite dimensional and can be computed without discretisation error.
...
We now present explicit examples of the computations in both \Cref{thm: norm bound smoothness,thm: norm bound L2} for the numerical results in \Cref{sec: numerics}. These examples are common in practical applications and we show the result of \Cref{thm: norm bound examples} in the main text to demonstrate that, while sometimes hard to compute by hand, the constants can estimated accurately in an efficient manner.
...
We present four numerical Lasso examples. The first two are in 1D to demonstrate the performance of different variants of \Cref{alg: refining FISTA}, both with and without adaptivity. In particular, we explore sparse Gaussian deconvolution and sparse signal recovery from Fourier data. We compare with the \emph{continuous basis pursuit} (CBP) discretisation \citep{Ekanadham2011, Duval2017b} which is also designed to achieve super-resolution accuracy within a convex framework. More details of this method will be provided in \Cref{sec: 1D Lasso examples}.
...
As shown in \Cref{sec: Lasso definition}, in 1D we have $\vars3_{\Varf0} = \vars3_{\Func0} = 2$. The estimates given in \eqref{eq: Lasso energy rate} and \eqref{eq: Lasso resolution rate} in dimension $d=1$ predict that the adaptive energy will decay at a rate of $\Func0_0(\varf0_n)\lesssim\frac1n$ so long as the pixel size also decreases at a rate of $\meshsize\sim\frac1n$. To achieve these rates, we implement a refinement criterion from \Cref{thm: practical refinement criteria} with guarantee of $\Func0_0(\varf0_{n_k-1})\lesssim 2^{-k}$ using the estimates made in \Cref{sec: Lasso gap and gradient}. We choose subspaces $\spcf0^n$ to approximately enforce
\begin{equation}
...
\end{equation}
is the maximum ratio of second vs. zeroth order Taylor approximations of $\A^*\vvard0_n$ on pixel $\domain_i^n$. This was found to be an efficient method of selecting pixels for refinement based on quantities which had already been computed. Note briefly that this greedy strategy directly targets uncertainty, refinements also happen outside of the support of $u_n$ to guarantee that this is representative of $\varf0^*$. Such refinement is necessary to avoid critical points of $\Func0$ which are not global minimisers.

\paragraph{Comparison of discretisation methods}
In \Cref{fig: convergence with ndofs} we compare the three core approaches: fixed uniform
...
\Cref{fig: convergence with method} compares many methods with either fixed or adaptive discretisations. Each adaptive scheme is allowed up to 1024 pixels and each uniform discretisation uses exactly 1024. The `Greedy FISTA' implementation was proposed by \citet{Liang2018} and we include the adaptive variant despite a lack of convergence proof. The remaining FISTA algorithms use a FISTA time step of $\vart1_n = \frac{n+a-1}{a}$ for the given value of $a$, as proposed by \citet{Chambolle2015}. In this example CBP used the greedy FISTA implementation which gave faster observed convergence. \Cref{fig: convergence with method} compares the discrete gaps because it is the accurate metric for fixed discretisations, and for the adaptive discretisation it should also be an accurate predictor of the continuous gap. 
...
\begin{itemize}
	...
	\item \Cref{thm: practical refinement criteria} accurately predicts the $\frac1n$ rate of the adaptive methods, mirrored in the fixed discretisations. This suggests that high-resolution discretisations are also initially limited by this $\frac1n$ rate before entering the asymptotic regime.
...
\begin{equation}
	\F{\Varx0}_i^I = \left\{\vvarx0\in[-\tfrac12,\tfrac12]^2\st \ip{\vvarx0}{\begin{pmatrix}\cos\theta_I\\\sin\theta_I\end{pmatrix}}\in \left[-\tfrac12+\tfrac{i-1}{50},-\tfrac12+\tfrac{i}{50}\right)\right\}, \quad \theta_I=\frac{180^\circ}{51}I \quad \text{ for } i,I=1,2,\ldots,50.
\end{equation}
This is not exactly in the form analysed by \Cref{thm: norm bound examples}, however for each $I$ the sets $\{\F{\Varx0}^I_i\st i\in[50]\}$ are disjoint therefore we can apply \Cref{thm: norm bound examples} block-wise to estimate
\begin{equation}
...
Finite elements are constructed with a mesh which provided a useful tool for adaptive refinement in \Cref{sec: bound continuous}. For wavelets, we will associate a tree with every discretisation and the leaves of the tree form a mesh. This perspective comes from the multi-resolution interpretation of wavelets. An example is seen in \Cref{fig: wavelet tree} for 1D Haar wavelets, $w_{j,k}(\varx0) = \sqrt{2}^k\vard1(2^{k}\varx0-j)$ where $\vard1 = \1_{[0,1)} - \1_{[-1,0)}$. 
...
In \Cref{sec: 1D Lasso examples} we used the continuous gap as a measure for convergence, for wavelets we will use the continuous gradient. With the tree structure we can easily adapt the results of \Cref{sec: Lasso gap and gradient} to estimate gradients (or function gaps). In particular,
\begin{align}
...
We consider two phantoms where $\varf0^\dagger$ is either a binary disc or the Shepp-Logan phantom. No noise is added to the Shepp-Logan data but \SI{5}{\percent} Gaussian white noise is added to the disc data. This is visualised in \Cref{fig: haar data}. All optimisations shown will be spatially adaptive using Haar wavelets and initialised with $\spcf0^0= \{\varx0\mapsto c\st c\in\F R\}$. The gradient metric shown throughout is the $\ell^2$ norm. Motivated by \eqref{eq: wavelet error metric}, the spatial adaptivity is chosen to refine nodes $j\in\op{leaf}(J_n)$ to ensure that 
...
The first numerical results shown in \Cref{fig: haar convergence} compare the same adaptive algorithms as shown in \Cref{fig: convergence with method}. In these examples we see that the greedy FISTA and the $a=20$ algorithms achieve almost linear convergence while $a=2$ is significantly slower.
...
\begin{figure}\centering
	...
	\caption{Discrete convergence of different implementations of \Cref{alg: refining FISTA} with an unlimited number of pixels.}\label{fig: haar convergence}
...
Our final application is a super-resolution/de-blurring inverse problem from biological microscopy. In mathematical terms, the observed data is a large number of sparse images which are corrupted by blurring and a large amount of noise, examples are seen in \Cref{fig: STORM data}. The task is to compute the centres of the spikes of signal in each image and then re-combine into a single super-resolved image, as in \Cref{fig: STORM results}. This technique is referred to as \emph{Single Molecule Localisation Microscopy} (SMLM), of which we consider the specific example of \emph{Stochastic Optical Reconstruction Microscopy} (STORM). Readers are directed to works such as by \citet{Sage2015,Sage2019,Schermelleh2019} for further details. The Lasso formulation has previously been shown to be effective in the context of STORM, see for instance \citet{Huang2017,Denoyelle2019}.
...
\begin{figure}\centering
	...
	\caption{Processed results of the STORM dataset. Top left: Lasso optimisation with \Cref{alg: refining FISTA}. Top right: Comparison with ThunderSTORM plugin. Bottom: Average data, no super-resolution or de-blurring.}\label{fig: STORM results}
\end{figure}
...
This section contains all of the statements and proofs of the results contained in \Cref{sec: FISTA convergence}.

\subsection{Proofs for Step 3}

\begin{theorem}[\Cref{thm: one step FISTA}]\label{app:thm: one step FISTA}
	\Paste{thm: one step FISTA}
	\begin{equation}
		\vart1_{n}^2(\Func0(\varf0_{n}) - \Func0(\varf2_n)) - (\vart1_{n}^2-\vart1_{n})(\Func0(\varf0_{n-1})-\Func0(\varf2_n)) \leq \tfrac1{2}\left[\norm{\varf1_{n-1}}^2-\norm{\varf1_{n}}^2\right] + \IP{\varf1_{n}-\varf1_{n-1}}{\varf2_n}.\label{app:eq: one-step FISTA}
	\end{equation}
\end{theorem}
\begin{proof}
	Modifying \cite[Theorem 2]{Chambolle2015}, for $n\geq1$ we apply \Cref{thm: descent lemma} with $\bar{\varf0} = \bar{\varf0}_{n-1}$ and $\varf2 = (1-\frac{1}{\vart1_{n}})\varf0_{n-1} + \frac{1}{\vart1_{n}}\varf2_n$. This gives
	\begin{equation}
...
\begin{theorem}[\Cref{thm: mini FISTA convergence}]\label{app:thm: mini FISTA convergence}
	\Paste{thm: mini FISTA convergence}
	\begin{multline}\label{app:eq: FISTA inequality}
		\vart1_N^2\Func0_0(\varf0_N) + \sum_{n=1}^{N-1}\rho_n \Func0_0(\varf0_n) + \frac{\norm{\varf1_N-\varf2_N}^2}{2} \leq \frac{\norm{\varf0_0-\varf2_0}^2-\norm{\varf2_0}^2+\norm{\varf2_N}^2}{2}
		\\ + \sum^N_{n=1} \vart1_n\Func0_0(\varf2_n) + \IP{\varf1_{n-1}}{\varf2_{n-1}-\varf2_n}.
	\end{multline}
\end{theorem}
\begin{proof}
	\Cref{app:thm: mini FISTA convergence} is just a summation of \eqref{app:eq: one-step FISTA} over all $n=1,\ldots,N$. To see this: first add and subtract $\Func0(\varf0^*)$ to each term on the left-hand side to convert $\Func0$ to $\Func0_0$, then move $\Func0_0(\varf2_n)$ to the right-hand side. Now \eqref{app:eq: one-step FISTA} becomes
	\begin{equation}
	...
	\end{equation}
	Noting that $\varf1_0=\varf0_0$, the previous two equations combine to prove the statement of \Cref{app:thm: mini FISTA convergence}.
\end{proof}
...
\begin{lemma}[\Cref{thm: mini exponential FISTA convergence}]\label{app:thm: mini exponential FISTA convergence}
...
\begin{proof}
	...
	\end{multline}
	By \Cref{app: tn upper bound}, $\vart1_n\leq n$ so we can further simplify
...
\begin{lemma}[\Cref{thm: sufficiently fast}]\label{app:thm: sufficiently fast}
	\Paste{thm: sufficiently fast}
\end{lemma}
\begin{proof}
	Starting from \Cref{app:thm: mini exponential FISTA convergence} we get
	\begin{align}
		\vart1_N^2\Func0_0(\varf0_N) + \tfrac12\norm{\varf1_N-\tilde{\varf2}_K}^2 &\leq C + \frac{\norm{\tilde{\varf2}_K}^2}{2} + \frac{(N+1)^2-n_K^2}{2}\Func0_0(\tilde{\varf2}_K) + \sum_{k=1}^K \frac{n_k^2-n_{k-1}^2}{2}\Func0_0(\tilde{\varf2}_{k-1}) + \IP{\varf1_{n_k-1}}{\tilde{\varf2}_k-\tilde{\varf2}_{k+1}}
		\\& \leq C + \frac{\norm{\tilde{\varf2}_K}^2}{2} + \frac{(N+1)^2}{2}\Func0_0(\tilde{\varf2}_K) \notag
		\\&\hspace{100pt}+\sum_{k=1}^K \frac{n_k^2}{2}\Func0_0(\tilde{\varf2}_{k-1}) + \IP{\varf1_{n_k-1}-\tilde{\varf2}_{k-1}+\tilde{\varf2}_{k-1}}{\tilde{\varf2}_k-\tilde{\varf2}_{k+1}}.
	\end{align}
	...
	\begin{description}
		...
		\begin{align}
			\vart1_N^2\Func0_0(\varf0_N) + \tfrac12\norm{\varf1_N-\tilde{\varf2}_K}^2 &\lesssim 1 + (N+1)^2\vars3_{\Func0}^{-K} + \sum_{k=1}^{K}n_k^2\vars3_{\Func0}^{-k} + (\norm{\varf1_{n_k-1}-\tilde{\varf2}_{k-1}} + 1)b_k
	...
	\end{description}
		...
		\begin{equation}
			\vart1_N^2\Func0_0(\varf0_N) \leq \frac{C_2^2}{2}\vars3_{\Varf0}^{2K}
		\end{equation}
		for all $N<n_K-1$.
\end{proof}

\subsection{Proof for Step 5}
\begin{lemma}[\Cref{thm: sufficiently slow}]\label{app:thm: sufficiently slow}
...
\begin{theorem}[\Cref{thm: stronger exponential FISTA convergence}]\label{app:thm: stronger exponential FISTA convergence}
	\Paste{thm: stronger exponential FISTA convergence}
\end{theorem}
\begin{proof}
	To apply \Cref{app:thm: sufficiently fast}, we need
	\begin{equation}
	...
	\end{equation}
	Therefore, the result of \Cref{app:thm: sufficiently fast} gives
	\begin{equation}
	...
	\begin{equation}
		\min_{n\leq N} \Func0_0(\varf0_n)\lesssim \splitln{\vars3_{\Func0}^{-k}}{N>n_k}{ \sfrac{\vars3_{\Varf0}^{2k}}{N^2}}{N\leq n_k} \lesssim N^{-2}\max(\vars3_{\Varf0}^{2k},\vars3_{\Varf0}^{2k}) \lesssim N^{-2(1-\kappa)}
	\end{equation}
	as required.
\end{proof}
...
\begin{proof}
	...
	\end{itemize}
	In each case, with $C$ sufficiently large, the induction holds. Now we can return to the precise condition of \Cref{app:thm: stronger exponential FISTA convergence}, $\Func0_0(\varf0_{n_k-1}) \leq C'\vars3_{\Func0}^{-k}$. Assume true for $k-1$. For each adaptive criterion:
	\begin{itemize}
	...
	\end{itemize}
	This completes the requirements of \Cref{app:thm: stronger exponential FISTA convergence}, therefore also this proof.
\end{proof}


\section{Proof of \texorpdfstring{\Cref{thm: generic a_U and a_E}}{rates for general finite element spaces}}\label{app: generic a_U and a_E}
The proof of \Cref{thm: generic a_U and a_E} is the result of the following three lemmas. The first, \Cref{app:thm: generic a_U bound}, is a general quantification of the equivalence between $L^q$ and $L^2$ norms on finite dimensional sub-spaces. A special case occurs when $q=1$ because the dual norm is a supremum rather than an integral. In \Cref{app:thm: specific a_U bound}, this locality is exploited by finite element spaces, which we assumed had a basis with local support. \Cref{app:thm: generic a_E bound} then performs the computations for the $\vars3_{\Func0}$ constant depending on the smoothness properties of $\Func0$.
...
\end{lemma}
\begin{proof}
	Most of the conditions of \Cref{app:thm: generic a_U bound} are already satisfied. Denote $C=\op{dim}(\tilde{\spcf0^0})$ and $\{e_j\st j\in[C]\}$ the standard orthonormal basis of $\tilde{\spcf0}^0$. The scaling properties of $\meshsize$-refining finite element spaces guarantee that the value of $C$ satisfies the conditions of \Cref{app:thm: generic a_U bound} and a basis of $\tilde{\spcf0}^k$ is given by
	\begin{equation}
	...
	We now compute the scaling constant in \Cref{app:thm: generic a_U bound}:
	\begin{equation}
	...
	\end{equation}
	This value is independent of $j$ and gives the desired bound as a result of \Cref{app:thm: generic a_U bound}.
\end{proof}
...
\begin{theorem}[\Cref{thm: norm bound examples}]\label{app:thm: norm bound examples}
	\Paste{thm: norm bound examples}
\end{theorem}
\begin{proof}[Proof case 1:]
	From \Cref{thm: norm bound L2} we have 
	\begin{equation}
...
\begin{proof}[Proof case 2:]
	...
	\end{equation}
	Now looking to apply \Cref{thm: norm bound smoothness}, note $\norm{\nabla^k\vard1_j}_\infty \leq \Vars3^k$, therefore
	\begin{align}
...
\begin{proof}[Proof case 3:]
	...
	Applying the results from \Cref{thm: norm bound smoothness}, note
	$$\begin{array}{rll}
	...
	Now, continuing the proof of \Cref{app:thm: norm bound examples}, for $\hat m=\sqrt[d]{m}$, $\delta=\frac{\sqrt{d}}{2}$ and $J=\{\vec{j}\in\F Z^d \st \norm{\vec{j}}_{\ell^\infty}\leq 2\hat m\}$, \Cref{app: exp sum bound} bounds 
	\begin{align*}
